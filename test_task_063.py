...
def test_task_063():
    ...
    clean_content = (
        "Hi John, I wanted to share some insights about your website performance."
    )
    ...
    spammy_content = (
        "CLICK HERE NOW!!! Make $1000 instantly!!! LIMITED TIME GUARANTEED!!!"
    )
    ...
    found_patterns = [
        ...
        if any(pattern in rule_id for rule_id in rule_ids)
    ]
    ...
    has_actionable = any(
        keyword in suggestions_text.lower() for keyword in suggestion_keywords
    )
    ...
    improved_result = checker.check_spam_score(
        improved_subject, improvements["improved_content"]
    )

    # Improved content should have lower score (with some tolerance)
    assert (
        improved_result.overall_score <= spammy_result.overall_score + 5
    ...
    assert (
        is_likely_spam("Professional email", "Business communication", threshold=50.0)
        == False
    )
