...
class EvidenceValidator:
    ...
    async def validate_acceptance_evidence(self, test_results: Dict[str, Any]) -> bool:
        ...
        test_passed = (
            test_results.get("status") == "passed" and
            test_results.get("exit_code") == 0 and
            test_results.get("tests_failed", 0) == 0
        )
    ...
    async def validate_deployment_evidence(self, deploy_results: Dict[str, Any]) -> bool:
        ...
        deploy_success = (
            deploy_results.get("status") == "success" and
            all(check.get("healthy", False) for check in deploy_results.get("health_check_results", []))
        )
            ...
            "true" if deploy_success else "false",
            {
                ...
                "validation_criteria": {
                    ...
                    "health_checks_passed": all(
                        ...
                        for check in deploy_results.get("health_check_results", [])
                    )
                }
            }
        )
