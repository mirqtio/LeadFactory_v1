...
class HealthChecker:
    ...
    async def check_health_endpoint(self) -> HealthCheckResult:
        ...
        return await self._check_endpoint(
            ...
            expected_patterns=["healthy", "ok", "status"]
        )
    ...
    async def check_liveness_endpoint(self) -> HealthCheckResult:
        ...
        return await self._check_endpoint(
            "liveness_endpoint", 
            self.config.liveness_endpoint,
            expected_patterns=["alive", "ok", "live"]
        )
        ...
        try:
            async with httpx.AsyncClient(timeout=self.config.timeout_seconds) as client:
                ...
                return HealthCheckResult(
                    ...
                    metadata={
                        "performance_level": (
                            "excellent" if response_time < 200 else 
                            "good" if response_time < 500 else
                            "fair" if response_time < 1000 else
                            "poor"
                        ),
