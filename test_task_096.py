...
async def test_pipeline_script():
    ...
    assert (
        criteria["businesses_processed"]["status"] == "PASS"
    ), "Should process businesses"
    ...
    print(
        f"   - Businesses processed: {report['execution_details']['businesses_processed']}"
    )
    print(f"   - Emails generated: {report['execution_details']['emails_generated']}")
    print(
        f"   - Metrics recorded: {report['execution_details']['metrics_recorded_count']}"
    )
    print(
        f"   - Execution time: {report['execution_details']['execution_time_seconds']}s"
    )
