...
from d7_storefront.checkout import CheckoutManager, CheckoutItem
...
def test_task_059():
    ...
    with patch.object(checkout_manager.stripe_client, 'create_checkout_session') as mock_create_session:
        mock_response = {
            "success": True,
    ...
    single_result = handler.handle_session_completed(single_report_event, "evt_single_123")
    ...
    with patch.object(checkout_manager.stripe_client, 'create_checkout_session') as mock_create:
    ...
    # Test idempotency
    processor_with_idempotency = WebhookProcessor(stripe_client, enable_idempotency=True)
    ...
    from d7_storefront.webhook_handlers import (
        PaymentIntentHandler, CustomerHandler, InvoiceHandler
    )
