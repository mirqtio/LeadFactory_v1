...
from d10_analytics.models import (
    FunnelEvent, MetricSnapshot, TimeSeriesData, FunnelConversion,
    FunnelStage, EventType, MetricType, AggregationPeriod, generate_uuid
)
...
class DailyMetricsAggregator:
    ...
    async def _build_stage_metrics(
        ...
        ).group_by(FunnelEvent.event_type).all()
        ...
        total_events = session.query(func.count(FunnelEvent.event_id)).filter(
            FunnelEvent.funnel_stage == stage,
            FunnelEvent.occurred_at >= start_datetime,
            FunnelEvent.occurred_at <= end_datetime,
        ...
        if total_events > 0:
            successful_events = session.query(func.count(FunnelEvent.event_id)).filter(
                FunnelEvent.funnel_stage == stage,
                FunnelEvent.occurred_at >= start_datetime,
                FunnelEvent.occurred_at <= end_datetime,
        ...
        avg_duration = session.query(func.avg(FunnelEvent.duration_ms)).filter(
            FunnelEvent.funnel_stage == stage,
            FunnelEvent.occurred_at >= start_datetime,
            FunnelEvent.occurred_at <= end_datetime,
    ...
    async def build_conversion_metrics(
        ...
        conversion_count = session.query(func.count(FunnelEvent.event_id)).filter(
        ...
        total_entries = session.query(func.count(FunnelEvent.event_id)).filter(
    ...
    async def build_cost_metrics(
        ...
        # Total cost in cents
        total_cost_cents = session.query(func.sum(FunnelEvent.cost_cents)).filter(
            FunnelEvent.occurred_at >= start_datetime,
            FunnelEvent.occurred_at <= end_datetime,
        ...
        event_count = session.query(func.count(FunnelEvent.event_id)).filter(
            FunnelEvent.occurred_at >= start_datetime,
            FunnelEvent.occurred_at <= end_datetime,
            FunnelEvent.cost_cents.isnot(None)
        ).scalar() or 0
    ...
    async def build_segment_metrics(
        ...
        ).group_by(FunnelEvent.campaign_id).all()
...
class FunnelCalculator:
    ...
    async def _calculate_stage_to_stage_conversion(
        ...
        started_count = session.query(func.count(func.distinct(FunnelEvent.session_id))).filter(
        ...
        completed_subquery = session.query(
            FunnelEvent.session_id
        ).filter(
    ...
    async def calculate_segment_funnels(
        ...
        campaigns = session.query(func.distinct(FunnelEvent.campaign_id)).filter(
            func.date(FunnelEvent.occurred_at) >= start_date,
            func.date(FunnelEvent.occurred_at) <= end_date,
            FunnelEvent.campaign_id.isnot(None)
        ).all()
    ...
    async def _calculate_campaign_funnel(
        ...
        for i in range(len(stages) - 1):
            ...
            completed_count = session.query(func.count(func.distinct(FunnelEvent.session_id))).filter(
                FunnelEvent.campaign_id == campaign_id,
                FunnelEvent.funnel_stage == to_stage,
                FunnelEvent.event_type == EventType.ENTRY,
                func.date(FunnelEvent.occurred_at) >= start_date,
    ...
    async def analyze_dropoffs(
        ...
        for stage in FunnelStage:
            ...
            entries = session.query(func.count(func.distinct(FunnelEvent.session_id))).filter(
                FunnelEvent.funnel_stage == stage,
                FunnelEvent.event_type == EventType.ENTRY,
                func.date(FunnelEvent.occurred_at) >= start_date,
                func.date(FunnelEvent.occurred_at) <= end_date,
            ...
            abandonments = session.query(func.count(func.distinct(FunnelEvent.session_id))).filter(
                FunnelEvent.funnel_stage == stage,
...
class CostAnalyzer:
    ...
    async def calculate_lead_costs(
        ...
        total_cost = session.query(func.sum(FunnelEvent.cost_cents)).filter(
            func.date(FunnelEvent.occurred_at) >= start_date,
            func.date(FunnelEvent.occurred_at) <= end_date,
            FunnelEvent.cost_cents.isnot(None)
        ).scalar() or 0
        ...
        total_leads = session.query(func.count(func.distinct(FunnelEvent.business_id))).filter(
            func.date(FunnelEvent.occurred_at) >= start_date,
            func.date(FunnelEvent.occurred_at) <= end_date,
    ...
    async def calculate_cpa_metrics(
        ...
        # Total conversions
        total_conversions = session.query(func.count(FunnelEvent.event_id)).filter(
            FunnelEvent.event_type == EventType.CONVERSION,
            func.date(FunnelEvent.occurred_at) >= start_date,
            func.date(FunnelEvent.occurred_at) <= end_date
        ).scalar() or 0
    ...
    async def calculate_roi_metrics(
        ...
        for stage in FunnelStage:
            stage_cost = session.query(func.sum(FunnelEvent.cost_cents)).filter(
                FunnelEvent.funnel_stage == stage,
                func.date(FunnelEvent.occurred_at) >= start_date,
                func.date(FunnelEvent.occurred_at) <= end_date,
                FunnelEvent.cost_cents.isnot(None)
            ...
            stage_events = session.query(func.count(FunnelEvent.event_id)).filter(
                FunnelEvent.funnel_stage == stage,
                func.date(FunnelEvent.occurred_at) >= start_date,
                func.date(FunnelEvent.occurred_at) <= end_date
            ).scalar() or 0
    ...
    async def calculate_efficiency_metrics(
        ...
        campaign_efficiency = session.query(
            FunnelEvent.campaign_id,
        ...
        for campaign_id, total_cost, total_events, successful_events in campaign_efficiency:
            if total_events > 0:
                # Cost per event
