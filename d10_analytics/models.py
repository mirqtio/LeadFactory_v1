...
class FunnelEvent(Base):
    ...
    occurred_at = Column(DateTime(timezone=True), nullable=False, index=True)  # When event actually occurred
...
class MetricSnapshot(Base):
    ...
    period_date = Column(Date, nullable=False, index=True)  # Date for daily aggregations
    ...
    # Metadata
    data_points = Column(Integer, nullable=False, default=0)  # Number of data points aggregated
    ...
    __table_args__ = (
        ...
        Index('idx_metrics_name_period_stage', 'metric_name', 'period_type', 'funnel_stage'),
        Index('idx_metrics_date_name_type', 'period_date', 'metric_name', 'metric_type'),
        Index('idx_metrics_campaign_period', 'campaign_id', 'period_start', 'period_end'),
        ...
        UniqueConstraint(
            'metric_name', 'funnel_stage', 'campaign_id', 'business_vertical', 
            'geography', 'period_type', 'period_start',
        ...
        CheckConstraint('confidence_score >= 0 AND confidence_score <= 1', name='ck_metrics_confidence_range'),
...
class TimeSeriesData(Base):
    ...
    metric_name = Column(String(255), nullable=False, index=True)
    series_name = Column(String(255), nullable=False, index=True)  # Unique series identifier
    ...
    __table_args__ = (
        ...
        # Constraints
        CheckConstraint('quality_score >= 0 AND quality_score <= 1', name='ck_timeseries_quality_range'),
    )
...
class DashboardMetric(Base):
    ...
    # Display formatting
    display_format = Column(String(50), nullable=True)  # currency, percentage, number, etc.
    ...
    # Time context
    time_period = Column(String(50), nullable=False)    # last_hour, today, this_week, etc.
    ...
    cache_ttl_seconds = Column(Integer, nullable=False, default=300)  # 5 minutes default
    ...
    __table_args__ = (
        ...
        Index('idx_dashboard_metrics_dashboard_order', 'dashboard_name', 'display_order'),
        ...
        UniqueConstraint('dashboard_name', 'widget_name', 'metric_name', name='uq_dashboard_widget_metric'),
        
        # Constraints
        CheckConstraint('decimal_places >= 0 AND decimal_places <= 10', name='ck_dashboard_decimal_places'),
        CheckConstraint('cache_ttl_seconds > 0', name='ck_dashboard_cache_ttl_positive'),
        CheckConstraint('calculation_duration_ms >= 0', name='ck_dashboard_duration_positive'),
    )
...
class FunnelConversion(Base):
    ...
    __table_args__ = (
        ...
        UniqueConstraint(
            'from_stage', 'to_stage', 'cohort_date', 'campaign_id', 'business_vertical',
            name='uq_funnel_conversion_dimensions'
        ),
        
        # Constraints
        CheckConstraint('started_count >= 0', name='ck_conversions_started_positive'),
        CheckConstraint('completed_count >= 0', name='ck_conversions_completed_positive'),
        CheckConstraint('completed_count <= started_count', name='ck_conversions_completed_valid'),
        CheckConstraint('conversion_rate >= 0 AND conversion_rate <= 1', name='ck_conversions_rate_range'),
        CheckConstraint('avg_time_to_convert_hours >= 0', name='ck_conversions_time_positive'),
    )