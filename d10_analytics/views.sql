...
-- =============================================================================
-- UNIT ECONOMICS MATERIALIZED VIEW (P2-010)
-- =============================================================================

-- Drop existing view if it exists
DROP MATERIALIZED VIEW IF EXISTS unit_economics_day CASCADE;
...
SELECT 
    ...
    -- LTV (Lifetime Value) - simplified as average revenue per customer
    CASE 
        WHEN COALESCE(dcv.total_conversions, 0) > 0 THEN 
...
-- Function to refresh unit economics materialized view
...
-- Update the refresh all function to include unit economics
CREATE OR REPLACE FUNCTION refresh_all_analytics_views()
RETURNS void AS $$
BEGIN
    PERFORM refresh_funnel_analysis_mv();
    PERFORM refresh_cohort_retention_mv();
    PERFORM refresh_unit_economics_day_mv();
END;
$$ LANGUAGE plpgsql;
