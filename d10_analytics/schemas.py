...
class SegmentFilter(BaseModel):
    ...
    campaign_ids: list[str] | None = Field(default=None, description="Filter by campaign IDs")
    business_verticals: list[str] | None = Field(default=None, description="Filter by business verticals")
    geographic_regions: list[str] | None = Field(default=None, description="Filter by geographic regions")
    funnel_stages: list[str] | None = Field(default=None, description="Filter by funnel stages")
...
class MetricsRequest(BaseModel):
    ...
    segment_filter: SegmentFilter | None = Field(default=None, description="Segment filters")
    metric_types: list[str] | None = Field(default=None, description="Types of metrics to retrieve (defaults to all)")
    aggregation_period: str | None = Field(default="daily", description="Aggregation period for time series data")
    include_breakdowns: bool = Field(default=False, description="Include segment breakdowns in response")
    limit: int | None = Field(default=1000, ge=1, le=10000, description="Maximum number of records to return")
...
class CohortAnalysisRequest(BaseModel):
    ...
    retention_periods: list[str] | None = Field(
...
class MetricDataPoint(BaseModel):
    ...
    segment_breakdown: dict[str, Any] | None = Field(default=None, description="Breakdown by segments")
...
class FunnelDataPoint(BaseModel):
    ...
    to_stage: str | None = Field(..., description="Target funnel stage")
    ...
    avg_time_to_convert_hours: Decimal | None = Field(
    ...
    cost_per_conversion_cents: int | None = Field(default=None, description="Cost per conversion")
...
class FunnelMetricsResponse(BaseModel):
    ...
    conversion_paths: list[dict[str, Any]] | None = Field(
        default=None, description="Detailed conversion paths (if requested)"
    )
    generated_at: datetime = Field(..., description="Response generation timestamp")
...
class HealthCheckResponse(BaseModel):
    ...
    metrics_count: int | None = Field(default=None, description="Total metrics records")
    last_aggregation: datetime | None = Field(default=None, description="Last successful aggregation timestamp")
