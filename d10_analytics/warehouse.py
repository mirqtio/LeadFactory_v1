...
from d10_analytics.aggregators import (
    DailyMetricsAggregator, FunnelCalculator, CostAnalyzer, SegmentBreakdownAnalyzer
)
...
class MetricsWarehouse:
        ...
        try:
            ...
            with get_db_session() as session:
                ...
                if not force_rebuild:
                    existing_metrics = session.query(MetricSnapshot).filter(
                ...
                conversion_metrics = await self.daily_aggregator.build_conversion_metrics(
                    session, target_date
                )
    ...
    async def calculate_funnel_metrics(
        ...
        try:
            ...
            with get_db_session() as session:
                ...
                segment_funnels = await self.funnel_calculator.calculate_segment_funnels(
                    session, start_date, end_date
                )
    ...
    async def build_segment_breakdowns(
        ...
        try:
            ...
            if segments is None:
                segments = ["geography", "business_vertical", "campaign", "funnel_stage"]
            ...
            with get_db_session() as session:
                ...
                for segment in segments:
                    ...
                    # Build funnel stage breakdowns
                        ...
                        stage_metrics = await self.segment_analyzer.build_stage_breakdown(
                            session, start_date, end_date
                        )
    ...
    def get_metrics_summary(
        ...
        with get_db_session() as session:
            ...
            ).group_by(MetricSnapshot.metric_type).all()
            ...
            ).group_by(FunnelEvent.funnel_stage).all()
            
            # Count conversions
            conversion_count = session.query(FunnelConversion).filter(
