...
OUTCOME_CRITERIA = {
    "P0-000": "`pytest --collect-only` exits 0 inside Docker **and** a checklist in README lists required versions (Python 3.11, Docker ≥ 20, Compose ≥ 2).",
    "P0-001": "`test_d4_coordinator.py` passes **and** coordinator returns freshest, deduped fields; cache key collisions across businesses impossible (property-based test).",
    "P0-002": "`smoke/test_full_pipeline_flow.py` generates PDF + email + DB rows for a sample URL within 90 s runtime.",
    "P0-003": "GitHub Actions logs show image build, KEEP suite green, coverage ≥ 80%, image pushed to GHCR.",
    "P0-004": "`alembic upgrade head` + autogen diff both return no changes on CI; downgrade path tested for latest revision.",
    "P0-005": "Running tests with `USE_STUBS=true` yields 0 external calls (network mocked); prod env rejects `USE_STUBS=true` at startup.",
    ...
    "P0-008": "Collect phase shows correct counts; `pytest -m slow` runs 0 tests in CI; import errors in ignored files eliminated.",
    "P0-009": "`git grep -i yelp` finds only comments/docs; migration drops last Yelp columns; stub server has no `/yelp/*` routes.",
    ...
    "P1-050": "Every external API request inserts a ledger row with `cost_usd`; daily aggregation view returns non-NULL totals.",
    ...
    "P2-040": "When ledger total > monthly cap, all flows transition to `Failed` with custom message; auto-resume next month verified.",
    "P2-050": "CI pipeline spins up Supabase shadow DB, runs migrations; deploy workflow switches `DATABASE_URL`; app health remains 200; old VPS Postgres container stopped."
}
