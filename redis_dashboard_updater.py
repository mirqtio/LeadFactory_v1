...
class RedisDashboardUpdater:
    ...
    def get_pm_status(self) -> List[Dict]:
        ...
        if not self.redis_available:
            for pm in pm_agents:
                pm.update({"status": "unknown", "current_prp": None, "last_heartbeat": None})
            return pm_agents
    ...
    def update_dashboard_html(self, template_path: str, output_path: str) -> bool:
        ...
        try:
            ...
            html_content = (
                template.replace("{{DASHBOARD_DATA}}", json.dumps(data, indent=2))
                .replace("{{TIMESTAMP}}", data["formatted_timestamp"])
                .replace("{{REDIS_STATUS}}", data["redis_metrics"]["redis_status"])
                .replace("{{TOTAL_PRPS}}", str(data["prp_metrics"]["total_prps"]))
                .replace("{{COMPLETION_RATE}}", f"{data['prp_metrics']['completion_rate']:.1%}")
                .replace(
                    "{{CURRENT_PRP}}",
                    data["prp_metrics"]["current_prp"]["id"] if data["prp_metrics"]["current_prp"] else "None",
                )
                .replace("{{SYSTEM_STATUS}}", data["system_health"]["overall_status"])
            )
