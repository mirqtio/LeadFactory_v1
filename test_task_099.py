...
def test_campaign_launch():
    ...
    result = subprocess.run(
        [sys.executable, str(launch_script), "--batch", "100", "--dry-run", "--json"],
        capture_output=True,
        text=True,
    )
    ...
    ), f"Errors in results: {results.get('errors')}"
...
def test_command_line_interface():
    ...
    result = subprocess.run(
        [sys.executable, str(launch_script), "--batch", "1", "--dry-run"],
        capture_output=True,
        text=True,
    )
    ...
    result = subprocess.run(
        [sys.executable, str(launch_script), "--batch", "1"],
        capture_output=True,
        text=True,
    )
    ...
    if result.returncode != 0:
        ...
        assert (
            "Pipeline components not available" in result.stdout
        ), f"Expected simulation mode message, got: {result.stdout}"
        assert (
            "CAMPAIGN LAUNCH: FAILED" in result.stdout
        ), "Should report failure when components missing"
        print(
            "✅ Task requirements command fails gracefully without pipeline components"
        )
    else:
    ...
    ), f"Dry-run mode failed: {dry_run_result.stderr}"
