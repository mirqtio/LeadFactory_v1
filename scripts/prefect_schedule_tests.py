...
async def create_deployments():
    ...
    return {"smoke": smoke_id, "heartbeat": heartbeat_id, "cleanup": cleanup_id}
...
async def main():
    ...
    if "--serve" in sys.argv:
        ...
        await serve(
            daily_smoke_flow.to_deployment(name="daily-smoke-test", cron="30 20 * * *"),
            heartbeat_flow.to_deployment(name="heartbeat-check", interval=7200),
            cleanup_flow.to_deployment(name="daily-cleanup", cron="0 3 * * *"),
        )
