...
class MonitoringTester:
    ...
    def test_prometheus_connectivity(self) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(
                    f"⚠️  Prometheus server responded with status {response.status_code}"
                )
                return False

        except requests.exceptions.ConnectionError:
            self.warnings.append(
                "Cannot connect to Prometheus - server may not be running"
            )
    ...
    def validate_alerts_configuration(self) -> bool:
        ...
        try:
            ...
            if missing_groups:
                self.warnings.append(
                    f"Missing recommended alert groups: {missing_groups}"
                )
            ...
            for group in groups:
                ...
                for rule in rules:
                    ...
                    if missing_fields:
                        self.errors.append(
                            f"Alert '{rule.get('alert', 'unnamed')}' missing fields: {missing_fields}"
                        )
                        continue
            ...
            key_metrics_patterns = [
                ...
                "leadfactory_memory_usage",
                "leadfactory_businesses_processed",
                "leadfactory_emails_",
                "leadfactory_payments_",
            ]
            ...
            if missing_metrics:
                self.warnings.append(
                    f"Some key metrics may not have alerts: {missing_metrics}"
                )
    ...
    def validate_dashboard_configuration(self) -> bool:
        ...
        try:
                ...
                for target in targets:
                    ...
                    if "leadfactory_" in expr:
                        ...
                        if metric_start >= 0:
                            ...
                            metric_name = (
                                metric_part.split()[0].split("(")[0].split("[")[0]
                            )
            ...
            print(
                f"   Metrics covered: {len(metrics_covered)} unique LeadFactory metrics"
            )
            ...
            if missing_sections:
                self.warnings.append(
                    f"Dashboard may be missing sections: {missing_sections}"
                )
    ...
    def test_key_metrics_availability(self) -> bool:
        ...
        try:
            ...
            response = requests.get(
                f"{self.prometheus_url}/api/v1/label/__name__/values", timeout=10
            )
            ...
            if response.status_code == 200:
                ...
                self.warnings.append(
                    f"Cannot query Prometheus metrics (status {response.status_code})"
                )
                return False
                    ...
                    try:
                        ...
                        if response.status_code == 200:
                            ...
                            if result.get("status") == "success":
                                ...
                                self.warnings.append(
                                    f"Alert '{rule.get('alert')}' has invalid expression: {error}"
                                )
                        else:
                            invalid_rules += 1
                        ...
                        invalid_rules += 1
                        self.warnings.append(
                            f"Cannot test rule '{rule.get('alert')}': {e}"
                        )
    ...
    def run_tests(self, skip_connectivity: bool = False) -> bool:
        ...
        if not skip_connectivity:
            ...
            if self.test_prometheus_connectivity():
                ...
                print(
                    "⏭️  Skipping Prometheus-dependent tests due to connectivity issues"
                )
        else:
