    ...
    def verify_yelp_integration(self) -> bool:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"✅ {service.upper()} API connected (response time: {response_time:.3f}s)")
                print(f"   Found {len(businesses)} businesses, {total_results} total available")
                ...
                if response.headers.get('RateLimit-Remaining'):
                    ...
                    if remaining < 100:
                        self.warnings.append(f"Yelp API rate limit low: {remaining} requests remaining")
...
def main():
    ...
    parser.add_argument("--use-stubs", action="store_true",
                       help="Use stub server instead of real APIs")
    ...
    if args.json:
        ...
        print(f"📊 Success Rate: {report['summary']['connected_services']}/{report['summary']['total_services']} services ({report['summary']['success_rate']:.1f}%)")
