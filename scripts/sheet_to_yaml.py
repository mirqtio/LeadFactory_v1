...
class SheetToYamlConverter:
    ...
    def fetch_sheet_data(self, sheet_id: str, tab: str) -> Dict[str, Any]:
        ...
        try:
            ...
            tier_result = self.service.spreadsheets().values().get(
                spreadsheetId=sheet_id, range=tier_range
            ).execute()
            tier_values = tier_result.get('values', [])
            
            # Fetch component configuration
            comp_range = f"{tab}!{COMPONENT_RANGE}"
            comp_result = self.service.spreadsheets().values().get(
                spreadsheetId=sheet_id, range=comp_range
            ).execute()
            comp_values = comp_result.get('values', [])
            
            # Fetch SHA from cell Z1
            sha_range = f"{tab}!Z1"
            sha_result = self.service.spreadsheets().values().get(
                spreadsheetId=sheet_id, range=sha_range
            ).execute()
        ...
        for row in values:
            ...
            if len(row) >= 2 and row[1] and not any(row[2:] if len(row) > 2 else []):
                ...
                try:
                    ...
                    components[current_component]['factors'][factor_name] = {
        ...
        for comp_name, comp_data in data['components'].items():
            if comp_data['factors']:
                ...
                if abs(factor_total - 1.0) > 0.05:
                    errors.append(
                        f"Component '{comp_name}' factor weights sum to {factor_total:.3f}, "
                        f"must be 1.0 ± 0.05"
                    )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Convert Google Sheets scoring configuration to YAML'
    )
    parser.add_argument(
        '--sheet-id',
        required=True,
