...
class ExperimentLoader:
    ...
    def validate_single_experiment(self, exp_id: str, exp_config: Dict[str, Any]) -> bool:
        ...
        for variant_id, variant_config in variants.items():
            ...
            if not isinstance(allocation, (int, float)) or allocation < 0 or allocation > 1:
                self.validation_errors.append(f"Experiment {exp_id}, variant {variant_id}: Invalid allocation {allocation}")
                is_valid = False
            total_allocation += allocation
        
        # Check total allocation sums to 1.0 (with small tolerance for floating point)
        if abs(total_allocation - 1.0) > 0.001:
            self.validation_errors.append(f"Experiment {exp_id}: Variant allocations sum to {total_allocation}, expected 1.0")
            is_valid = False
        ...
        for exp_id, exp_config in experiments.items():
            ...
            if len(variants) == 2:
                ...
                if all(abs(alloc - 0.5) < 0.001 for alloc in allocations):
                    ...
                    self.validation_warnings.append(f"Experiment {exp_id}: Not a 50/50 split (allocations: {allocations})")
    ...
    def load_all_experiments(self) -> bool:
        ...
        success = (report['validation']['valid'] and 
                  report['summary']['all_criteria_met'] and 
                  system_load_success)
...
def main():
    ...
    parser.add_argument("--config-dir", default="experiments",
