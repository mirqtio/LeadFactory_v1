...
def validate_basic_functionality(client: TestClient) -> bool:
    ...
    try:
        ...
        print_result("Health endpoint returns 200", passed, 
    ...
    print_result("Status is 'ok' when healthy", passed, 
...
def validate_database_check(client: TestClient) -> bool:
    ...
    print_result("Database status is valid", valid_status,
                f"Status: {db_check.get('status')}")
    
    # Check latency if connected
    if db_check.get("status") == "connected":
        has_latency = "latency_ms" in db_check
        print_result("Has latency measurement", has_latency,
                    f"Latency: {db_check.get('latency_ms', 'N/A')}ms")
...
def validate_performance(client: TestClient, num_requests: int = 20) -> bool:
    ...
    print_result("Average internal time < 100ms", passed,
                f"Average: {avg_internal:.2f}ms")
    all_passed &= passed
    
    passed = max_internal < 100
    print_result("Max internal time < 100ms", passed,
                f"Max: {max_internal:.2f}ms")
    all_passed &= passed
    
    passed = p95_response < 200  # Allow some overhead for HTTP
    print_result("95th percentile response < 200ms", passed,
                f"P95: {p95_response:.2f}ms")
    all_passed &= passed
...
def validate_error_handling(client: TestClient) -> bool:
    ...
    try:
        ...
        print_result("Returns valid status code", passed,
                    f"Status: {response.status_code}")
...
def validate_detailed_endpoint(client: TestClient) -> bool:
    ...
    try:
        ...
        passed = response.status_code in [200, 503]
        print_result("Detailed endpoint exists", passed,
                    f"Status: {response.status_code}")
