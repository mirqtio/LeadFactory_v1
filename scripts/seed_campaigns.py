...
class CampaignSeeder:
    ...
    def validate_target_data(self, targets: List[Dict[str, Any]]) -> bool:
        ...
        required_fields = [
            'target_name', 'vertical', 'location', 'zip_code', 
            'radius_miles', 'daily_quota', 'priority', 'description'
        ]
        ...
        for i, target in enumerate(targets):
            ...
            if target['vertical'] not in valid_verticals:
                self.errors.append(f"{target_name}: Invalid vertical '{target['vertical']}', must be one of: {valid_verticals}")
            
            # Validate priority
            if target['priority'] not in valid_priorities:
                self.errors.append(f"{target_name}: Invalid priority '{target['priority']}', must be one of: {valid_priorities}")
    ...
    def create_target_campaigns(self, targets: List[Dict[str, Any]]) -> bool:
        ...
        try:
            ...
            existing_values = [value[0] for value in self.session.execute(
            ...
            for target_data in targets:
                ...
                target = Target(
                    ...
                    priority_score=0.8 if target_data['priority'] == 'high' else 0.5 if target_data['priority'] == 'medium' else 0.3,
                    is_active=True
                )
        ...
        try:
            ...
            if len(targets) == len(self.created_targets):
                ...
                for target in targets:
                    ...
                    print(f"   ZIP {target[0]}: {target[1]} vertical (businesses: {target[3]}, priority: {target[4]:.1f}, {status})")
...
def main():
    ...
    if args.json:
        ...
        print(f"📊 Success Rate: {report['summary']['targets_created']}/{report['summary']['targets_processed']} campaigns ({report['summary']['success_rate']:.1f}%)")
