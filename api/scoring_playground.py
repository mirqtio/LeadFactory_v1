...
def get_current_weights() -> tuple[list[WeightVector], str]:
    ...
    for name, config in weights_data.get("weights", {}).items():
        weights.append(WeightVector(name=name, weight=config.get("weight", 0.0), description=config.get("description")))
...
def get_sample_leads(db: Session, count: int = 100) -> list[Lead]:
    ...
    # Check cache
    if (
        _sample_leads_cache is not None
        and _cache_timestamp is not None
        and time.time() - _cache_timestamp < CACHE_DURATION
    ):
        return _sample_leads_cache[:count]

    # Query sample leads
    leads = db.query(Lead).filter(Lead.is_deleted.is_(False), Lead.is_manual.is_(False)).limit(count).all()
...
async def get_weights() -> dict[str, Any]:
    ...
    return {"weights": weights, "sha": sha, "total": sum(w.weight for w in weights)}


@router.post("/weights/import", response_model=WeightImportResponse)
async def import_weights_to_sheets(request: WeightImportRequest, db: Session = Depends(get_db)) -> WeightImportResponse:
    """
    ...
    return WeightImportResponse(sheet_url=sheet_url, weights_count=len(weights), sha=sha)


@router.post("/score/delta", response_model=ScoreDeltaResponse)
async def calculate_score_deltas(request: ScoreDeltaRequest, db: Session = Depends(get_db)) -> ScoreDeltaResponse:
    """
...
async def propose_weight_changes(proposal: ProposeDiffRequest, db: Session = Depends(get_db)) -> ProposeDiffResponse:
    """
    ...
    try:
        ...
        weights_dict = {
            w.name: {"weight": w.weight, "description": w.description or f"Weight for {w.name}"}
            for w in proposal.new_weights
        }

        new_yaml_content = yaml.dump(
            {"weights": weights_dict, "version": "2.0", "updated_at": datetime.utcnow().isoformat()},
        ...
        diff_output = subprocess.check_output(["git", "diff", "HEAD", str(weights_path)], text=True)
        ...
        return ProposeDiffResponse(
            pr_url=pr_url, branch_name=branch_name, commit_sha=commit_sha[:8], yaml_diff=diff_output
        )
