...
reload_requests = Counter(
    "internal_reload_requests_total", "Total number of internal reload requests", ["endpoint", "status"]
)

reload_duration = Histogram("internal_reload_duration_seconds", "Time spent processing reload requests")
...
async def reload_scoring_rules(auth: bool = Depends(get_internal_auth)) -> dict[str, Any]:
    ...
    try:
        ...
        if not isinstance(engine, ConfigurableScoringEngine):
            raise HTTPException(status_code=501, detail="Scoring engine does not support hot reload")
        ...
        raise HTTPException(status_code=500, detail=f"Failed to reload scoring rules: {str(e)}")
...
async def scoring_health_check(auth: bool = Depends(get_internal_auth)) -> dict[str, Any]:
    ...
    try:
        ...
        test_data = {"company_info": {"name_quality": True}, "online_presence": {"website_quality": True}}
