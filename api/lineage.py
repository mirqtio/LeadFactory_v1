...
def get_lineage_stats(db: Session = Depends(get_db)) -> Dict[str, Any]:
    ...
    return {
        ...
        "template_distribution": [{"version": row[0], "count": row[1]} for row in template_stats],
...
def get_lineage_by_report_id(report_id: str, db: Session = Depends(get_db)) -> Dict[str, Any]:
    ...
    lineage = db.query(ReportLineage).filter(ReportLineage.report_generation_id == report_id).first()

    if not lineage:
        raise HTTPException(status_code=404, detail="Lineage not found")

    # Log access for audit
    audit = ReportLineageAudit(lineage_id=lineage.id, action="view_lineage", accessed_at=datetime.utcnow())
    db.add(audit)
    db.commit()
...
def get_pipeline_logs(lineage_id: str, db: Session = Depends(get_db)) -> Dict[str, Any]:
    ...
    # Log access
    audit = ReportLineageAudit(lineage_id=lineage.id, action="view_logs", accessed_at=datetime.utcnow())
    db.add(audit)
    db.commit()
...
def download_raw_inputs(lineage_id: str, db: Session = Depends(get_db)):
    """
    ...
    # Log download
    audit = ReportLineageAudit(lineage_id=lineage.id, action="download_raw_inputs", accessed_at=datetime.utcnow())
    db.add(audit)
    db.commit()
    ...
    return Response(
        ...
        headers={"Content-Disposition": f'attachment; filename="lineage_{lineage_id}_raw_inputs.json.gz"'},
    )
...
def delete_lineage(lineage_id: str, db: Session = Depends(get_db)) -> Dict[str, str]:
    ...
    # Log deletion
    audit = ReportLineageAudit(lineage_id=lineage.id, action="delete_lineage", accessed_at=datetime.utcnow())
    db.add(audit)
