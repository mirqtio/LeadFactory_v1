...
from account_management.models import AccountUser, APIKey, Organization, Team, UserStatus
...
async def register(request: Request, data: UserRegister, db: Session = Depends(get_db)):
    ...
    existing_user = db.query(AccountUser).filter(AccountUser.email == data.email.lower()).first()
    ...
    if data.username:
        existing_username = db.query(AccountUser).filter(AccountUser.username == data.username.lower()).first()
    ...
    if data.organization_name:
        ...
        organization = Organization(name=data.organization_name, slug=org_slug, billing_email=data.email)
        db.add(organization)
        db.flush()
    ...
    if organization:
        default_team = Team(name="Default Team", slug="default", organization_id=organization.id, is_default=True)
        db.add(default_team)
        db.commit()
    ...
    return AuthTokenResponse(
        access_token=access_token, refresh_token=refresh_token, user=UserResponse.model_validate(user)
    )
...
async def login(request: Request, data: UserLogin, db: Session = Depends(get_db)):
    ...
    user = AuthService.authenticate_user(db, data.email, data.password.get_secret_value())
    ...
    # Create session
    access_token, refresh_token, session = AuthService.create_session(
        db, user, ip_address=request.client.host, user_agent=request.headers.get("User-Agent"), device_id=data.device_id
    )
...
@router.post("/logout")
async def logout(current_user: AccountUser = Depends(get_current_user), db: Session = Depends(get_db)):
...
async def reset_password(data: PasswordReset, db: Session = Depends(get_db)):
    ...
    user = AuthService.reset_password(db, data.token, data.new_password.get_secret_value())
...
async def get_current_user_profile(
    current_user: AccountUser = Depends(get_current_user), db: Session = Depends(get_db)
):
...
async def update_profile(
    data: UserUpdate, current_user: AccountUser = Depends(get_current_user), db: Session = Depends(get_db)
):
    ...
    if data.username and data.username != current_user.username:
        existing = (
            ...
            .filter(AccountUser.username == data.username.lower(), AccountUser.id != current_user.id)
            .first()
        )
...
async def change_password(
    data: PasswordChange, current_user: AccountUser = Depends(get_current_user), db: Session = Depends(get_db)
):
    """Change current user password"""
    # Verify current password
    if not AuthService.verify_password(data.current_password.get_secret_value(), current_user.password_hash):
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Current password is incorrect")

    # Update password
    current_user.password_hash = AuthService.hash_password(data.new_password.get_secret_value())
    db.commit()
...
async def get_current_organization(
    ...
    if not current_user.organization_id:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="User is not part of an organization")
...
async def create_organization(
    data: OrganizationCreate, current_user: AccountUser = Depends(get_current_user), db: Session = Depends(get_db)
):
    ...
    # Create default team
    default_team = Team(name="Default Team", slug="default", organization_id=org.id, is_default=True)
    db.add(default_team)
...
async def get_organization_stats(current_user: AccountUser = Depends(get_current_user), db: Session = Depends(get_db)):
    """Get organization statistics"""
    if not current_user.organization_id:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="User is not part of an organization")

    # Get stats
    total_users = db.query(AccountUser).filter(AccountUser.organization_id == current_user.organization_id).count()

    active_users = (
        db.query(AccountUser)
        .filter(AccountUser.organization_id == current_user.organization_id, AccountUser.status == UserStatus.ACTIVE)
        .count()
    )

    total_teams = db.query(Team).filter(Team.organization_id == current_user.organization_id).count()

    total_api_keys = db.query(APIKey).filter(APIKey.organization_id == current_user.organization_id).count()

    active_api_keys = (
        db.query(APIKey)
        .filter(APIKey.organization_id == current_user.organization_id, APIKey.is_active == True)
        .count()
    )
...
async def create_team(
    data: TeamCreate, current_user: AccountUser = Depends(get_current_user), db: Session = Depends(get_db)
):
    """Create a new team"""
    if not current_user.organization_id:
    ...
    # Check slug uniqueness
    existing = (
        db.query(Team).filter(Team.organization_id == current_user.organization_id, Team.slug == data.slug).first()
    )

    if existing:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Team slug already exists in organization")

    # Create team
    team = Team(**data.model_dump(), organization_id=current_user.organization_id)
    db.add(team)
    db.commit()
    db.refresh(team)
...
async def get_team(team_id: str, current_user: AccountUser = Depends(get_current_user), db: Session = Depends(get_db)):
    """Get team details"""
...
async def create_api_key(
    data: APIKeyCreate, current_user: AccountUser = Depends(get_current_user), db: Session = Depends(get_db)
):
    ...
    # Create API key
    raw_key, api_key = AuthService.create_api_key(db, current_user, data.name, data.scopes, expires_at)
...
async def revoke_api_key(
    key_id: str, current_user: AccountUser = Depends(get_current_user), db: Session = Depends(get_db)
):
    """Revoke an API key"""
