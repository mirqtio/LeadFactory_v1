...
class EnterpriseShim:
    def __init__(self, agent_type: AgentType, session: str, window: str, 
                 queue_name: str, redis_url: str):
    ...
    def send_to_tmux(self, message: str):
        ...
        try:
            ...
            cmd = [
                "tmux", "send-keys", "-t", f"{self.session}:{self.window}",
                message
            ]
    ...
    def blmove_process(self):
        ...
        while self.running:
            try:
                ...
                cmd = [
                    "redis-cli", "-u", os.environ.get('REDIS_URL', 'redis://localhost:6379'),
                    "BLMOVE", self.queue_name, inflight_queue, "RIGHT", "LEFT", "5"
                ]
...
def main():
    ...
    parser.add_argument('--agent-type', required=True, 
                       choices=['orchestrator', 'pm', 'validator', 'integrator'],
                       help='Type of agent')
