...
class WebhookHandler:
    ...
    def process_events(self, events: List[WebhookEvent]) -> Dict[str, Any]:
        ...
        logger.info(
            f"Webhook processing complete: {results['processed']} processed, {results['errors']} errors, {results['skipped']} skipped"
        )
        return results
        ...
        try:
            with SessionLocal() as session:
                ...
                if delivery:
                    ...
                    logger.warning(
                        f"No delivery found for message ID {event.message_id}, skipping open event recording"
                    )
    ...
    def _handle_unsubscribe_event(self, event: WebhookEvent) -> bool:
        ...
        try:
            # Add to suppression list
            self.compliance_manager.record_suppression(
                email=event.email, reason="user_unsubscribe", source="sendgrid_webhook"
            )
            ...
            with SessionLocal() as session:
                ...
                if delivery:
                    ...
                    logger.warning(
                        f"No delivery found for message ID {event.message_id}, skipping unsubscribe event recording"
                    )
        ...
        try:
            with SessionLocal() as session:
                ...
                if delivery:
                    ...
                    logger.warning(
                        f"No delivery found for message ID {event.message_id}, skipping processed event recording"
                    )
