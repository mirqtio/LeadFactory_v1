    ...
    def get_suppression_stats(self) -> Dict[str, Any]:
        ...
        try:
            with SessionLocal() as session:
                ...
                total_active = session.query(SuppressionList).filter(
                    SuppressionList.is_active == True
                ).count()
                
                # By source (since no list_type field)
                unsubscribe_source_count = session.query(SuppressionList).filter(
                    and_(
                        SuppressionList.is_active == True,
                ...
                # By reason
                unsubscribe_count = session.query(SuppressionList).filter(
                    and_(
                        SuppressionList.is_active == True,
                ...
                bounce_count = session.query(SuppressionList).filter(
                    and_(
                        SuppressionList.is_active == True,
