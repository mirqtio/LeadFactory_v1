...
def test_ab_experiments():
    ...
    assert "EXPERIMENT LOADING: SUCCESS" in result.stdout, "Loader script did not report success"
    assert "Subject Line Test Configured" in result.stdout, "Subject line test not detected by loader"
    assert "Price Point Test Configured" in result.stdout, "Price point test not detected by loader"
    assert "50 50 Split Configured" in result.stdout, "50/50 split not detected by loader"
    ...
    for exp_id, exp_config in experiments.items():
        ...
        for variant_id, variant_config in variants.items():
            assert "allocation" in variant_config, f"Variant {variant_id} missing allocation"
            allocation = variant_config["allocation"]
            assert 0.0 <= allocation <= 1.0, f"Invalid allocation for {variant_id}: {allocation}"
