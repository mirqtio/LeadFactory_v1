...
from .rules_schema import validate_rules, ScoringRulesSchema
...
scoring_rules_default_used = Gauge(
    'scoring_rules_default_used',
    'Whether default scoring rules are being used (1) or not (0)'
)
...
class ScoringEngine:
            ...
            if not self.config_path.exists():
                ...
                if os.getenv('ENV') == 'production':
                    raise FileNotFoundError(
                        f"Scoring rules file required in production: {self.config_path}"
                    )

                # For local dev/tests, use defaults
                logger.warning(
                    f"Scoring rules file not found: {self.config_path}. "
                    "Using default configuration for development."
                )
                self._load_defaults()
    ...
    def calculate_score(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for comp_name, comp_config in components.items():
            ...
            comp_score = self._calculate_component_score(
                comp_data, comp_config, comp_name
            )
    ...
    def get_tier_thresholds(self) -> Dict[str, float]:
        ...
        return {
            tier.label: tier.min
            for tier in self._schema.tiers.values()
        }
