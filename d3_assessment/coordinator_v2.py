...
class AssessmentCoordinatorV2:
    ...
    async def assess_business(
        ...
        available_assessors = [
            name for name in assessor_names 
            if name in self.assessors
        ]
            ...
            async with semaphore:
                try:
                    ...
                    result = await asyncio.wait_for(
                        assessor.assess(url, business_data),
                        timeout=timeout
                    )
    ...
    async def assess_batch(
        ...
        """
        tasks = [
            self.assess_business(business, assessor_names)
