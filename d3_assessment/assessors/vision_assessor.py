...
class VisionAssessor(BaseAssessor):
    ...
    async def _get_client(self) -> OpenAIClient:
        """Get or create OpenAI client"""
        if not self._client:
            self._client = await get_provider("openai")
        return self._client
    ...
    async def assess(self, url: str, business_data: Dict[str, Any]) -> AssessmentResult:
        ...
        try:
            ...
            screenshot_url = (
                business_data.get('screenshot_url') or
                business_data.get('assessments', {}).get('screenshot_url')
            )
