...
# Import models
from database.models import Target, Business, ScoringResult
...
async def test_yelp_to_assessment_flow(
    ...
    with patch('d3_assessment.pagespeed.PageSpeedAnalyzer.analyze') as mock_pagespeed, \
    ...
    test_db_session.refresh(test_business)
    assessment_from_db = test_db_session.query(AssessmentResult).filter_by(
...
async def test_data_consistency_verified(
    ...
    assessment_count = test_db_session.query(AssessmentResult).filter_by(
        business_id=test_business.id
    ).count()
    ...
    test_db_session.refresh(test_business)
    related_assessments = test_db_session.query(AssessmentResult).filter_by(
        business_id=test_business.id
    ).all()
...
async def test_performance_benchmarked(
    ...
    assert pipeline_total_time < MAX_PIPELINE_TIME_SECONDS, \
...
async def test_complete_sourcing_to_scoring_integration(
    ...
    test_db_session.refresh(business)
    db_assessment = test_db_session.query(AssessmentResult).filter_by(
