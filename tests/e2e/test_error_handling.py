...
from database.models import (
    Business, Target, Batch, Email, Purchase, 
    BatchStatus, EmailStatus, PurchaseStatus, GeoType
)
...
def test_partial_results_saved(test_db_session):
    ...
    for scenario in partial_processing_scenarios:
        ...
        if 'assessment' in scenario['name']:
            saved_count = test_db_session.query(AssessmentResult).filter(
                AssessmentResult.id.like(f"{scenario['name']}_item_%")
            ).count()
        elif 'email' in scenario['name']:
            saved_count = test_db_session.query(Email).filter(
                Email.id.like(f"{scenario['name']}_item_%")
            ).count()
        elif 'report' in scenario['name']:
            saved_count = test_db_session.query(ReportGeneration).filter(
                ReportGeneration.id.like(f"{scenario['name']}_item_%")
            ).count()
        elif 'pipeline' in scenario['name']:
            saved_count = test_db_session.query(PipelineRun).filter(
                PipelineRun.id.like(f"{scenario['name']}_item_%")
            ).count()
        else:
    ...
    assert len(successful_recoveries) >= 3, f"Expected at least 3 successful recoveries, got {len(successful_recoveries)}"
...
def test_retries_work_properly(test_db_session):
    ...
    for result in retry_results:
        status = "✅" if result['behavior_correct'] else "❌"
        print(f"  {status} {result['scenario']}: {result['actual_attempts']} attempts, success: {result['final_success']}")
...
def test_no_data_corruption(test_db_session):
    ...
    corruption_scenarios = [
        ...
        },
        {
            ...
            'operations': [
                'delete_business',
                'create_email_for_deleted_business'
            ],
    ...
    for scenario in corruption_scenarios:
        ...
        try:
            ...
            for i, operation in enumerate(scenario['operations']):
                try:
                    ...
                    should_fail = (
                        'failure_point' in scenario and 
                        i == scenario['failure_point']
                    )
        ...
        try:
            ...
            orphaned_emails = test_db_session.query(Email).filter(
                ~Email.business_id.in_(
            ...
            duplicate_businesses = test_db_session.query(Business.yelp_id).group_by(Business.yelp_id).having(
                test_db_session.query(Business.yelp_id).count() > 1
            ).count()
            ...
            invalid_assessments = test_db_session.query(AssessmentResult).filter(
            ...
            if scenario['type'] == 'transaction_rollback':
                ...
                transaction_records = test_db_session.query(Email).filter(
                    Email.subject == "Consistency Test Email"
                ).count()
    ...
    assert len(successful_corruption_tests) >= 5, f"Expected at least 5 corruption tests, got {len(successful_corruption_tests)}"
    ...
    for result in stress_test_results:
        ...
        print(f"  {status} {result['scenario']}: {result['operations_successful']}/{result['operations_attempted']} successful")
