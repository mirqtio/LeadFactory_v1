...
from database.models import (
    Business, Target, Batch, Purchase, Email, EmailClick, EmailSuppression,
    WebhookEvent, ScoringResult, GatewayUsage, PurchaseStatus, EmailStatus,
    GeoType, BatchStatus
)
from d6_reports.models import ReportGeneration, ReportDelivery, ReportStatus, ReportType, DeliveryMethod
...
def test_all_domains_integrate(
    ...
    assessment_data = {
        ...
        "llm_insights": ["Improve mobile responsiveness", "Add contact forms", "Optimize images"]
    }
...
def test_metrics_recorded(
    ...
    # Pipeline metrics
    pipeline_efficiency = (pipeline.records_processed / (pipeline.records_processed + pipeline.records_failed) * 100) if (pipeline.records_processed + pipeline.records_failed) > 0 else 0
    ...
    print(f"Pipeline: {pipeline.records_processed} processed, {pipeline.records_failed} failed ({pipeline_efficiency:.1f}% efficiency)")
