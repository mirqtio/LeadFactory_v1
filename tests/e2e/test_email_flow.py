...
def test_personalization_works(
    ...
    assert not any(spam_word in personalized_content['subject'].lower() 
                  for spam_word in ['urgent', 'limited time', 'act now'])
    assert not any(spam_word in personalized_content['body'].lower()
...
def test_compliance_verified(
    ...
    suppressed_emails = test_db_session.query(EmailSuppression).filter_by(
        email_hash=email_hash
    ).all()
    ...
    bounced_emails = test_db_session.query(EmailSuppression).filter_by(
        email_hash=bounce_email_hash
    ).all()
...
def test_tracking_confirmed(
    ...
    recorded_clicks = test_db_session.query(EmailClick).filter_by(
        email_id=test_email.id
    ).all()
    ...
    from database.models import EmailStatus
    campaign_emails = test_db_session.query(Email).filter(
        Email.business_id == test_business.id
    ).all()
    
    sent_count = len([e for e in campaign_emails if e.status in [EmailStatus.SENT, EmailStatus.OPENED, EmailStatus.CLICKED]])
    opened_count = len([e for e in campaign_emails if e.status in [EmailStatus.OPENED, EmailStatus.CLICKED]])
    clicked_count = test_db_session.query(EmailClick).join(Email).filter(
        Email.business_id == test_business.id
    ).count()
