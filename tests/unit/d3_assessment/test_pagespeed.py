...
from d3_assessment.pagespeed import (  # noqa: E402
    PageSpeedAssessor, PageSpeedBatchAssessor
)
...
class TestTask031AcceptanceCriteria:
            ...
            "lighthouseResult": {
                ...
                "audits": {
                    ...
                    "max-potential-fid": {
                        ...
                        "displayValue": "50 ms",
                        "description": (
                            "The maximum potential First Input Delay.")
                    },
                    ...
                    "render-blocking-resources": {
                        ...
                        "description": (
                            "Resources are blocking the first paint "
                            "of your page."),
                        "details": {
                    ...
                    "uses-text-compression": {
                        ...
                        "description": (
                            "Text-based resources should be served "
                            "with compression."),
                        "details": {}
                    }
                }
    ...
    @pytest.mark.asyncio
    async def test_core_web_vitals_extracted(
            self, assessor, mock_pagespeed_result):
        """
        ...
        """
        with patch.object(
                assessor.client, 'analyze_url',
                new_callable=AsyncMock) as mock_analyze:
            with patch.object(
                    assessor.client, 'calculate_cost',
                    new_callable=AsyncMock) as mock_cost:
    ...
    @pytest.mark.asyncio
    async def test_all_scores_captured(
            self, assessor, mock_pagespeed_result):
        """
    ...
    @pytest.mark.asyncio
    async def test_issue_extraction_works(
            self, assessor, mock_pagespeed_result):
        """
        ...
        with patch.object(
            ...
            with patch.object(
                ...
                opportunities = assessor._extract_opportunities(
                ...
                diagnostics = assessor._extract_diagnostics(
                    mock_pagespeed_result)
    ...
    @pytest.mark.asyncio
    async def test_mobile_first_approach(
            self, assessor, mock_pagespeed_result):
        """
        ...
        with patch.object(
            ...
            with patch.object(
                ...
                mobile_categories = mock_pagespeed_result[
    ...
    @pytest.mark.asyncio
    async def test_mobile_only_assessment(
            self, assessor, mock_pagespeed_result):
    ...
    @pytest.mark.asyncio
    async def test_assessment_cost_tracking(
            self, assessor, mock_pagespeed_result):
    ...
    def test_domain_extraction(self, assessor):
        ...
        assert assessor._extract_domain(
    ...
    async def test_batch_assessor(self, mock_pagespeed_result):
        ...
        batch_assessor = PageSpeedBatchAssessor(
            api_key="test-key", max_concurrent=2)
        ...
        with patch.object(
                batch_assessor.assessor.client, 'analyze_url',
                new_callable=AsyncMock) as mock_analyze:
            with patch.object(
                    batch_assessor.assessor.client, 'calculate_cost',
                    new_callable=AsyncMock) as mock_cost:
                mock_analyze.return_value = mock_pagespeed_result
                mock_cost.return_value = Decimal("0.00")

                results = await batch_assessor.assess_multiple_websites(
                    websites)

                assert len(results) == 2
                assert all(isinstance(result, AssessmentResult)
    ...
    @pytest.mark.asyncio
    async def test_comprehensive_assessment_flow(
            self, assessor, mock_pagespeed_result):
...
if __name__ == "__main__":
    ...
    async def run_tests():
        ...
        try:
            ...
            # Run all tests
            await test_instance.test_core_web_vitals_extracted(
                assessor, mock_result)
            await test_instance.test_all_scores_captured(
                assessor, mock_result)
            await test_instance.test_issue_extraction_works(
                assessor, mock_result)
            await test_instance.test_mobile_first_approach(
                assessor, mock_result)
            await test_instance.test_mobile_only_assessment(
                assessor, mock_result)
            await test_instance.test_assessment_cost_tracking(
                assessor, mock_result)
