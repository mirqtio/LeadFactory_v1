...
from d3_assessment.pagespeed import PageSpeedAssessor, PageSpeedBatchAssessor
from d3_assessment.models import AssessmentResult, PageSpeedAssessment
...
class TestTask031AcceptanceCriteria:
    ...
    async def test_core_web_vitals_extracted(self, assessor, mock_pagespeed_result):
        ...
        with patch.object(assessor.client, 'analyze_url', new_callable=AsyncMock) as mock_analyze:
            with patch.object(assessor.client, 'calculate_cost', new_callable=AsyncMock) as mock_cost:
                ...
                assert assessment.first_input_delay == 50  # FID (from max-potential-fid)
    ...
    async def test_issue_extraction_works(self, assessor, mock_pagespeed_result):
        ...
        with patch.object(assessor.client, 'analyze_url', new_callable=AsyncMock) as mock_analyze:
            with patch.object(assessor.client, 'calculate_cost', new_callable=AsyncMock) as mock_cost:
                ...
                assert len(opportunities) == 2  # unused-css-rules and render-blocking-resources
                
                # Check high-impact opportunity first (sorted by savings)
                high_impact = opportunities[0]
                assert high_impact["id"] == "unused-css-rules"  # 1200ms savings
                ...
                assert medium_impact["id"] == "render-blocking-resources"  # 800ms savings
                ...
                assert len(diagnostics) == 1  # uses-text-compression (no savings)
    ...
    async def test_assessment_cost_tracking(self, assessor, mock_pagespeed_result):
        ...
        with patch.object(assessor.client, 'analyze_url', new_callable=AsyncMock) as mock_analyze:
            with patch.object(assessor.client, 'calculate_cost', new_callable=AsyncMock) as mock_cost:
                mock_analyze.return_value = mock_pagespeed_result
                mock_cost.return_value = Decimal("0.004")  # $0.004 per request
                ...
                assert assessment.total_cost_usd == Decimal("0.008")  # 2 requests * $0.004
