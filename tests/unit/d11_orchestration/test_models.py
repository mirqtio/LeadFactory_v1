...
from d11_orchestration.models import (
    PipelineRun, PipelineRunStatus, PipelineType,
    Experiment, ExperimentStatus, ExperimentVariant, VariantType,
    VariantAssignment, PipelineTask, ExperimentMetric,
    generate_uuid
)
...
class TestOrchestrationModels:
    ...
    def test_status_management(self, session, sample_pipeline_run):
        ...
        tasks = session.query(PipelineTask).filter_by(pipeline_run_id=pipeline_run.run_id).order_by(PipelineTask.execution_order).all()
