...
from d11_orchestration.flows.bucket_enrichment import (
...
class TestBucketEnrichmentFlow:
    ...
    def test_update_entity_buckets(self, mock_update, mock_session):
        ...
        enriched_entities = {
            'businesses': [
                {'id': 'biz-1', 'geo_bucket': 'high-high-high', 'vert_bucket': 'medium-medium-low'}
            ],
            'targets': [
                {'id': 'tgt-1', 'geo_bucket': 'low-low-low', 'vert_bucket': 'high-high-medium'}
            ],
            'stats': {}
        }
    ...
    def test_bucket_enrichment_flow(self, mock_update, mock_enrich, mock_get_entities, mock_load):
        ...
        mock_enrich.return_value = {
            'businesses': [{'id': 'biz-1', 'geo_bucket': 'high-high-high', 'vert_bucket': 'medium-medium-low'}],
            'targets': [{'id': 'tgt-1', 'geo_bucket': 'low-low-low', 'vert_bucket': 'high-high-medium'}],
            'stats': {
        ...
        with patch('d11_orchestration.flows.bucket_enrichment.get_bucket_loader') as mock_get_loader:
            ...
            entities = {
                'businesses': [
                    ...
                    {'id': 'b2', 'zip_code': '88888', 'categories': ['unknown2', 'unknown3']}
                ],
                'targets': []
            }
