...
def test_impact_coefficients_exist():
    ...
    yaml_path = (
        Path(__file__).parent.parent.parent / "config" / "impact_coefficients.yaml"
    )
    ...
    for category in expected_categories:
        ...
        for severity in [1, 2, 3, 4]:
            assert (
                severity in coefficients[category]
            ), f"Severity {severity} should exist for category '{category}'"

            # Check coefficient is a positive number
            beta = coefficients[category][severity]
            assert isinstance(
                beta, (int, float)
            ), f"Coefficient for {category}[{severity}] should be numeric"
            assert (
                beta > 0
            ), f"Coefficient for {category}[{severity}] should be positive"
...
def test_coefficient_ordering():
    ...
    for category, severities in coefficients.items():
        ...
        for severity in [1, 2, 3, 4]:
            ...
            assert (
                beta > prev_beta
            ), f"Coefficients should increase with severity in {category}"
            prev_beta = beta
...
def test_performance_critical_value():
    ...
    assert (
        coefficients["performance"][4] == 0.0060
    ), "Performance critical should match Google/Soasta 3% uplift"
