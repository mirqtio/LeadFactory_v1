...
class TestPrerequisiteValidator:
    ...
    def test_check_dependencies_installed_exception(self, validator, mock_subprocess):
        """Test dependencies check with exception."""
        mock_subprocess.side_effect = Exception("pip error")
    ...
    def test_check_pytest_collection_exception(self, validator, mock_subprocess):
        """Test pytest collection check with exception."""
        mock_subprocess.side_effect = Exception("pytest error")
    ...
    def test_check_docker_build_exception(self, validator, mock_subprocess):
        """Test Docker build check with exception."""
        mock_subprocess.side_effect = Exception("build error")
                            ...
                            with patch.object(validator, 'check_dependencies_installed', return_value=successful_check):
                                with patch.object(validator, 'check_pytest_collection', return_value=successful_check):
                                    with patch.object(validator, 'check_ci_toolchain', return_value=successful_check):
                                        with patch.object(validator, 'check_docker_build', return_value=successful_check):
...
class TestModuleFunctions:
    ...
    def test_print_results_success(self, capsys):
        ...
        result = PrerequisiteResult(
            checks=[
                PrerequisiteCheck(name="Test Check", passed=True, required=True, message="✅ Test passed")
            ],
            environment_info={
                "python_version": "3.11.0",
                "platform": "darwin",
    ...
    def test_print_results_failure(self, capsys):
        ...
        result = PrerequisiteResult(
            checks=[
                PrerequisiteCheck(name="Test Check", passed=False, required=True, message="❌ Test failed")
            ],
            environment_info={
        ...
        with patch('core.prerequisites.sys.argv', test_args):
            with patch('core.prerequisites.PrerequisiteValidator') as mock_validator_class:
                ...
                mock_result = PrerequisiteResult(
                    checks=[
                        PrerequisiteCheck(name="Test", passed=True, required=True, message="✅ Test passed")
                    ]
                )
                mock_validator.validate_all_prerequisites.return_value = mock_result
                mock_validator_class.return_value = mock_validator
    ...
    def test_cli_main_failure(self, capsys):
        ...
        with patch('core.prerequisites.sys.argv', test_args):
            with patch('core.prerequisites.PrerequisiteValidator') as mock_validator_class:
                ...
                mock_result = PrerequisiteResult(
                    checks=[
                        PrerequisiteCheck(name="Test", passed=False, required=True, message="❌ Test failed")
                    ]
                )
                mock_validator.validate_all_prerequisites.return_value = mock_result
                mock_validator_class.return_value = mock_validator
...
class TestEdgeCases:
    ...
    def test_prerequisite_result_with_invalid_checks(self):
        ...
        # This should still work as checks is optional
        result = PrerequisiteResult(checks=None)
