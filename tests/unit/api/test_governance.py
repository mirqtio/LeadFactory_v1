...
from database.governance_models import AuditLog, User, UserRole
...
class TestRoleChecker:
    ...
    def test_admin_allowed_for_admin_role(self):
        ...
        admin_user = User(id="admin-id", email="admin@test.com", name="Admin", role=UserRole.ADMIN, is_active=True)
    ...
    def test_viewer_blocked_for_admin_role(self):
        ...
        viewer_user = User(id="viewer-id", email="viewer@test.com", name="Viewer", role=UserRole.VIEWER, is_active=True)
...
class TestAuditLogging:
    ...
    async def test_audit_log_creation(self):
        ...
        mock_user = User(id="user-id", email="user@test.com", name="Test User", role=UserRole.ADMIN)
...
class TestUserManagement:
    ...
    async def test_create_user_success(self):
        ...
        mock_db = Mock(spec=Session)
        mock_request = Mock()
        mock_admin = User(id="admin-id", email="admin@test.com", role=UserRole.ADMIN)
        ...
        user_data = UserCreate(email="newuser@test.com", name="New User", role=UserRole.VIEWER)

        # Mock the create_audit_log to avoid async issues
        with patch("api.governance.create_audit_log"):
            result = await create_user(request=mock_request, user_data=user_data, db=mock_db, current_user=mock_admin)
    ...
    async def test_create_user_duplicate_email(self):
        ...
        user_data = UserCreate(email="existing@test.com", name="New User", role=UserRole.VIEWER)

        with pytest.raises(HTTPException) as exc_info:
            await create_user(request=mock_request, user_data=user_data, db=mock_db, current_user=mock_admin)
    ...
    async def test_change_role_success(self):
        ...
        role_change = RoleChangeRequest(new_role=UserRole.ADMIN, reason="Promoted to admin for project management")

        with patch("api.governance.create_audit_log"):
            result = await change_user_role(
                request=mock_request, user_id="target-id", role_change=role_change, db=mock_db, current_user=mock_admin
            )
    ...
    async def test_prevent_self_demotion(self):
        ...
        role_change = RoleChangeRequest(new_role=UserRole.VIEWER, reason="Testing self-demotion")

        with pytest.raises(HTTPException) as exc_info:
            await change_user_role(
                request=mock_request, user_id="admin-id", role_change=role_change, db=mock_db, current_user=mock_admin
            )
    ...
    async def test_deactivate_user_success(self):
        ...
        target_user = User(id="target-id", email="target@test.com", is_active=True)
        mock_db.query().filter().first.return_value = target_user

        with patch("api.governance.create_audit_log"):
            result = await deactivate_user(
                request=mock_request, user_id="target-id", db=mock_db, current_user=mock_admin
            )
    ...
    async def test_prevent_self_deactivation(self):
        ...
        with pytest.raises(HTTPException) as exc_info:
            await deactivate_user(request=mock_request, user_id="admin-id", db=mock_db, current_user=mock_admin)
...
class TestAuditQuerying:
    ...
    async def test_query_audit_logs(self):
        ...
        query = AuditLogQuery(action="CREATE", object_type="User", limit=100, offset=0)

        result = await query_audit_logs(query=query, db=mock_db, current_user=mock_user)
