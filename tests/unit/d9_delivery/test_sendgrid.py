...
from d9_delivery.sendgrid_client import (
    SendGridClient, EmailData, SendGridResponse,
    send_single_email, create_email_data
)
from d9_delivery.email_builder import (
    EmailBuilder, EmailTemplate, PersonalizationData,
    create_personalization_data, build_audit_email
)
...
class TestUtilityFunctions:
    ...
    def test_build_audit_email(self):
        ...
        email = build_audit_email(
            ...
            issues=[
                {"title": "Test Issue", "suggestion": "Fix it"}
            ],
            score=90.0,
...
def test_integration():
    ...
    with patch.dict('os.environ', {'SENDGRID_API_KEY': 'test_key'}):
        ...
        assert payload['personalizations'][0]['to'][0]['email'] == "integration@example.com"
