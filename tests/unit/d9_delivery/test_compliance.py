...
from d9_delivery.models import SuppressionList, EmailDelivery, DeliveryEvent
from d9_delivery.compliance import (
    ComplianceManager, UnsubscribeToken, ComplianceHeaders,
    check_email_suppression, generate_unsubscribe_link, process_unsubscribe_request
)
...
class TestComplianceEnhancements:
    ...
    def test_compliance_header_url_encoding(self, compliance_manager):
        ...
        assert "user%2Btag%40example.com" in headers.list_unsubscribe or "user+tag@example.com" in headers.list_unsubscribe
    ...
    def test_suppression_stats_query_optimization(self, compliance_manager):
        ...
        with patch('d9_delivery.compliance.SessionLocal') as mock_session_local:
            ...
            mock_session.query.return_value.filter.return_value.count.side_effect = [50, 10, 15, 5]
    ...
    def test_concurrent_unsubscribe_processing(self, mock_session_local, compliance_manager):
        ...
        mock_session_local.return_value.__enter__.return_value = mock_session
        mock_session.query.return_value.filter.return_value.first.return_value = None
        mock_session.commit.side_effect = [IntegrityError("duplicate", None, None), None]
