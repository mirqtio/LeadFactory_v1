...
from d9_delivery.webhook_handler import (
    WebhookHandler, WebhookEvent, SendGridEventType,
    process_sendgrid_webhook, create_test_webhook_event
)
...
class TestWebhookHandler:
    ...
    def test_process_delivered_event(self, webhook_handler, sample_email_delivery):
        ...
        with SessionLocal() as session:
            delivery = session.query(EmailDelivery).filter(
                EmailDelivery.sendgrid_message_id == sample_email_delivery["message_id"]
            ).first()
            ...
            event_record = session.query(DeliveryEvent).filter(
    ...
    def test_process_bounce_event(self, webhook_handler, sample_email_delivery):
        ...
        with SessionLocal() as session:
            ...
            bounce = session.query(BounceTracking).filter(
                BounceTracking.email == "test@example.com"
            ).first()
...
class TestIntegration:
    ...
    def test_full_webhook_processing_flow(self):
        ...
        with SessionLocal() as session:
            ...
            delivery = session.query(EmailDelivery).filter(
                EmailDelivery.sendgrid_message_id == "integration_msg"
            ).first()
            assert delivery.status == DeliveryStatus.DELIVERED.value
            assert delivery.delivered_at is not None
            ...
            events = session.query(DeliveryEvent).filter(
                DeliveryEvent.sendgrid_message_id == "integration_msg"
            ).all()
            assert len(events) == 2
    ...
    def test_bounce_with_suppression_flow(self):
        ...
        with SessionLocal() as session:
            ...
            delivery = session.query(EmailDelivery).filter(
                EmailDelivery.sendgrid_message_id == "bounce_msg"
            ).first()
            assert delivery.status == DeliveryStatus.BOUNCED.value
            
            # Check bounce tracking created
            bounce = session.query(BounceTracking).filter(
                BounceTracking.email == "bounce_test@example.com"
            ).first()
            assert bounce is not None
            assert bounce.bounce_type == BounceType.HARD.value
