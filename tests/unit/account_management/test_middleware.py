...
class TestAuthMiddleware:
    ...
    async def test_get_current_user_from_token_inactive_user(self, db: Session):
        ...
        inactive_user = AccountUser(id="inactive123", email="inactive@example.com", status=UserStatus.INACTIVE)
    ...
    async def test_authenticate_request_api_key_authorization(self, db: Session, valid_user: AccountUser, mock_request):
        ...
        with patch.object(AuthMiddleware, "get_current_user_from_token") as mock_token_auth, patch.object(
            AuthMiddleware, "get_current_user_from_api_key"
        ) as mock_api_auth:
    ...
    async def test_authenticate_request_all_methods_fail(self, db: Session, mock_request):
        ...
        mock_request.headers = {"Authorization": "Bearer invalid_token", "X-API-Key": "invalid_key"}
    ...
    async def test_authenticate_request_bearer_token_priority(self, db: Session, valid_user: AccountUser, mock_request):
        ...
        mock_request.headers = {"Authorization": "Bearer valid_token", "X-API-Key": "valid_api_key"}
...
class TestPermissionChecker:
    ...
    def valid_user(self, db: Session):
        ...
        user = AccountUser(id="user123", email="test@example.com", status=UserStatus.ACTIVE, organization_id="org123")
        db.add(user)
        db.commit()
        return user
    ...
    async def test_permission_checker_success(
        self, db: Session, valid_user: AccountUser, mock_request, permission_checker
    ):
        """Test successful permission check"""
        with patch.object(AuthMiddleware, "authenticate_request") as mock_auth, patch.object(
            permission_checker, "check_user_permission"
        ) as mock_check:
    ...
    async def test_permission_checker_permission_denied(
        self, db: Session, valid_user: AccountUser, mock_request, permission_checker
    ):
        ...
        with patch.object(AuthMiddleware, "authenticate_request") as mock_auth, patch.object(
            ...
            with patch("account_management.middleware.get_db") as mock_get_db:
                ...
                audit_log = (
                    db.query(AccountAuditLog)
                    .filter(AccountAuditLog.user_id == valid_user.id, AccountAuditLog.action == "UNAUTHORIZED_READ")
                    .first()
                )
...
class TestOrganizationMemberChecker:
    ...
    def no_org_user(self, db: Session):
        ...
        user = AccountUser(id="user456", email="noorg@example.com", status=UserStatus.ACTIVE, organization_id=None)
        db.add(user)
        db.commit()
        return user
    ...
    async def test_organization_member_checker_no_org_allowed(
        self, db: Session, no_org_user: AccountUser, mock_request
    ):
...
class TestAuditLogger:
    ...
    async def test_log_action_basic(self, db: Session, valid_user: AccountUser, mock_request):
        ...
        with patch("account_management.middleware.get_db") as mock_get_db:
            ...
            audit_log = (
                db.query(AccountAuditLog)
                .filter(AccountAuditLog.user_id == valid_user.id, AccountAuditLog.action == "CREATE")
                .first()
            )
    ...
    async def test_log_action_minimal(self, db: Session, valid_user: AccountUser, mock_request):
        ...
        with patch("account_management.middleware.get_db") as mock_get_db:
            ...
            await AuditLogger.log_action(
                request=mock_request, user=valid_user, action="READ", resource_type="report", db=db
            )

            # Verify audit log was created
            audit_log = (
                db.query(AccountAuditLog)
                .filter(AccountAuditLog.user_id == valid_user.id, AccountAuditLog.action == "READ")
                .first()
            )
...
class TestDependencyHelpers:
    ...
    async def test_require_organization_member_no_org(self, db: Session, mock_request):
        ...
        no_org_user = AccountUser(
            id="user456", email="noorg@example.com", status=UserStatus.ACTIVE, organization_id=None
        )
...
class TestMiddlewareIntegration:
    ...
    def mock_request_with_auth(self):
        ...
        request = MagicMock(spec=Request)
        request.headers = {"Authorization": "Bearer valid_token", "User-Agent": "test-client"}
    ...
    async def test_full_authentication_flow(self, db: Session, org_user: AccountUser, mock_request_with_auth):
        ...
        with patch("account_management.middleware.AuthService.decode_token") as mock_decode:
            mock_decode.return_value = {"type": "access", "sub": org_user.id}
    ...
    async def test_permission_check_with_audit_logging(
        self, db: Session, org_user: AccountUser, mock_request_with_auth
    ):
        ...
        with patch.object(AuthMiddleware, "authenticate_request") as mock_auth, patch.object(
            ...
            with patch("account_management.middleware.get_db") as mock_get_db:
                ...
                audit_log = (
                    db.query(AccountAuditLog)
                    .filter(AccountAuditLog.user_id == org_user.id, AccountAuditLog.action == "UNAUTHORIZED_DELETE")
                    .first()
                )
    ...
    async def test_organization_member_check_integration(
        self, db: Session, org_user: AccountUser, mock_request_with_auth
    ):
