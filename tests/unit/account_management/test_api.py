...
from account_management.api import get_current_user, get_current_user_optional, make_aware, router, utc_now
from account_management.auth_service import AuthService
from account_management.models import AccountUser, APIKey, Organization, Team, UserSession, UserStatus
...
class TestDependencies:
    ...
    async def test_get_current_user_invalid_token_type(self, db: Session, valid_credentials):
        ...
        with patch.object(AuthService, "decode_token") as mock_decode:
            mock_decode.return_value = {"type": "refresh", "sub": "user123"}  # Wrong type
    ...
    async def test_get_current_user_user_not_found(self, db: Session, valid_credentials):
        ...
        with patch.object(AuthService, "decode_token") as mock_decode:
            mock_decode.return_value = {"type": "access", "sub": "nonexistent_user"}
    ...
    async def test_get_current_user_inactive_user(self, db: Session, valid_credentials):
        ...
        with patch.object(AuthService, "decode_token") as mock_decode:
            mock_decode.return_value = {"type": "access", "sub": inactive_user.id}
    ...
    async def test_get_current_user_optional_valid_credentials(self, db: Session, valid_user: AccountUser):
        ...
        with patch.object(AuthService, "decode_token") as mock_decode:
            mock_decode.return_value = {"type": "access", "sub": valid_user.id}
...
class TestAuthEndpoints:
    ...
    def test_register_success(self, db: Session, mock_request):
        ...
        with patch("account_management.api.AuthService.hash_password") as mock_hash, patch(
            "account_management.api.AuthService.create_session"
        ) as mock_session, patch("account_management.api.AuthService.create_email_verification_token") as mock_verify:
    ...
    def test_register_duplicate_email(self, db: Session, mock_request):
        ...
        existing_user = AccountUser(
            email="existing@example.com", password_hash="hashed_password", status=UserStatus.ACTIVE
        )
        db.add(existing_user)
        db.commit()

        user_data = UserRegister(
            email="existing@example.com", password="SecurePass123!", username="newuser"  # Same email
        )
    ...
    def test_register_duplicate_username(self, db: Session, mock_request):
        ...
        user_data = UserRegister(
            email="new@example.com", password="SecurePass123!", username="existinguser"  # Same username
        )
    ...
    def test_login_success(self, db: Session, mock_request):
        ...
        mock_user = AccountUser(id="user123", email="test@example.com", status=UserStatus.ACTIVE)

        with patch("account_management.api.AuthService.authenticate_user") as mock_auth, patch(
            "account_management.api.AuthService.create_session"
        ) as mock_session:
    ...
    def test_refresh_token_success(self, db: Session, mock_request):
        ...
        user = AccountUser(id="user123", email="test@example.com", status=UserStatus.ACTIVE)
        db.add(user)
        db.commit()

        refresh_data = RefreshTokenRequest(refresh_token="valid_refresh_token")

        with patch("account_management.api.AuthService.decode_token") as mock_decode, patch(
            "account_management.api.AuthService.create_session"
        ) as mock_session:
    ...
    def test_refresh_token_invalid_type(self, db: Session, mock_request):
        ...
        refresh_data = RefreshTokenRequest(refresh_token="access_token_not_refresh")

        with patch("account_management.api.AuthService.decode_token") as mock_decode:
            mock_decode.return_value = {"type": "access", "sub": "user123"}  # Wrong type
    ...
    def test_refresh_token_user_not_found(self, db: Session, mock_request):
        ...
        with patch("account_management.api.AuthService.decode_token") as mock_decode:
            mock_decode.return_value = {"type": "refresh", "sub": "nonexistent_user"}
    ...
    def test_verify_email_success(self, db: Session):
        ...
        verification_data = EmailVerificationRequest(token="valid_verification_token")
    ...
    def test_reset_password_success(self, db: Session):
        ...
        reset_data = PasswordReset(token="valid_reset_token", new_password="NewSecurePass123!")
    ...
    def test_reset_password_invalid_token(self, db: Session):
        ...
        reset_data = PasswordReset(token="invalid_token", new_password="NewSecurePass123!")
...
class TestUserProfileEndpoints:
    ...
    def current_user(self, db: Session):
        ...
        user = AccountUser(
            id="user123", email="test@example.com", username="testuser", full_name="Test User", status=UserStatus.ACTIVE
        )
        db.add(user)
        db.commit()
        return user
    ...
    def test_update_profile_username_taken(self, db: Session, current_user: AccountUser):
        ...
        other_user = AccountUser(
            id="other123", email="other@example.com", username="takenusername", status=UserStatus.ACTIVE
        )
        db.add(other_user)
        db.commit()
    ...
    def test_update_profile_same_username(self, db: Session, current_user: AccountUser):
        ...
        update_data = UserUpdate(username="testuser", full_name="Updated Name")  # Same as current username
    ...
    def test_change_password_success(self, db: Session, current_user: AccountUser):
        ...
        change_data = PasswordChange(current_password="current_password", new_password="NewSecurePass123!")

        with patch("account_management.api.AuthService.verify_password") as mock_verify, patch(
            "account_management.api.AuthService.hash_password"
        ) as mock_hash:
    ...
    def test_change_password_wrong_current(self, db: Session, current_user: AccountUser):
        ...
        change_data = PasswordChange(current_password="wrong_password", new_password="NewSecurePass123!")
...
class TestOrganizationEndpoints:
    ...
    def org_user(self, db: Session):
        ...
        org = Organization(id="org123", name="Test Organization", slug="test-org", billing_email="billing@test.com")
        db.add(org)
        db.flush()

        user = AccountUser(id="user123", email="test@example.com", organization_id=org.id, status=UserStatus.ACTIVE)
        db.add(user)
        db.commit()
    ...
    def no_org_user(self, db: Session):
        ...
        user = AccountUser(id="user456", email="noorg@example.com", status=UserStatus.ACTIVE)
        db.add(user)
        db.commit()
        return user
    ...
    def test_create_organization_success(self, db: Session, no_org_user: AccountUser):
        ...
        org_data = OrganizationCreate(name="New Organization", slug="new-org", billing_email="billing@new.com")
    ...
    def test_create_organization_user_has_org(self, db: Session, org_user: AccountUser):
        ...
        org_data = OrganizationCreate(
            name="Another Organization", slug="another-org", billing_email="billing@another.com"
        )
    ...
    def test_create_organization_slug_exists(self, db: Session, no_org_user: AccountUser):
        ...
        existing_org = Organization(name="Existing Org", slug="existing-slug", billing_email="existing@test.com")
        db.add(existing_org)
        db.commit()

        org_data = OrganizationCreate(
            name="New Organization", slug="existing-slug", billing_email="billing@new.com"  # Same slug
        )
    ...
    def test_get_organization_stats(self, db: Session, org_user: AccountUser):
        ...
        user2 = AccountUser(
            id="user2", email="user2@test.com", organization_id=org_user.organization_id, status=UserStatus.ACTIVE
        )
        user3 = AccountUser(
            id="user3",
        ...
        # Add teams
        team1 = Team(name="Team 1", slug="team1", organization_id=org_user.organization_id)
        team2 = Team(name="Team 2", slug="team2", organization_id=org_user.organization_id)
        db.add_all([team1, team2])

        # Add API keys
        api_key1 = APIKey(
            id="key1", name="Key 1", user_id=org_user.id, organization_id=org_user.organization_id, is_active=True
        )
        api_key2 = APIKey(
            id="key2",
            name="Key 2",
...
class TestTeamEndpoints:
    ...
    def org_user_with_teams(self, db: Session):
        ...
        org = Organization(id="org123", name="Test Organization", slug="test-org")
        db.add(org)
        db.flush()
        ...
        team1 = Team(id="team1", name="Team Alpha", slug="alpha", organization_id=org.id)
        team2 = Team(id="team2", name="Team Beta", slug="beta", organization_id=org.id)
        db.add_all([team1, team2])
        db.commit()
    ...
    def test_create_team_success(self, db: Session, org_user_with_teams: AccountUser):
        ...
        team_data = TeamCreate(name="New Team", slug="new-team", description="A new team for testing")
    ...
    def test_create_team_slug_exists(self, db: Session, org_user_with_teams: AccountUser):
        ...
        team_data = TeamCreate(name="Another Alpha", slug="alpha")  # Slug already exists
    ...
    def test_get_team_wrong_org(self, db: Session):
        ...
        other_org = Organization(id="other_org", name="Other Org", slug="other")
        db.add(other_org)
        db.flush()

        other_team = Team(id="other_team", name="Other Team", slug="other", organization_id=other_org.id)
        db.add(other_team)

        # Create user in different organization
        user = AccountUser(
            id="user456", email="other@example.com", organization_id="org123", status=UserStatus.ACTIVE  # Different org
        )
        db.add(user)
        db.commit()
...
class TestAPIKeyEndpoints:
    ...
    def test_create_api_key_success(self, db: Session, org_user_with_keys: AccountUser):
        ...
        key_data = APIKeyCreate(name="New API Key", scopes=["read", "write"], expires_in_days=30)
    ...
    def test_create_api_key_no_org(self, db: Session):
        ...
        key_data = APIKeyCreate(name="New API Key", scopes=["read"])
    ...
    def test_create_api_key_with_expiration(self, db: Session, org_user_with_keys: AccountUser):
        ...
        key_data = APIKeyCreate(name="Expiring Key", scopes=["read"], expires_in_days=7)

        with patch("account_management.api.AuthService.create_api_key") as mock_create, patch(
            "account_management.api.utc_now"
        ) as mock_now:
            mock_now.return_value = datetime(2025, 1, 1, tzinfo=UTC)
            expected_expiry = datetime(2025, 1, 8, tzinfo=UTC)

            mock_api_key = APIKey(id="expiring_key", name="Expiring Key", expires_at=expected_expiry)
    ...
    def test_revoke_api_key_wrong_user(self, db: Session):
        ...
        # Create another user
        other_user = AccountUser(id="other_user", email="other@example.com", status=UserStatus.ACTIVE)
        db.add(other_user)
        db.commit()
