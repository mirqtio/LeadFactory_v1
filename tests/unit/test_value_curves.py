...
def test_file_exists():
    assert DATA_PATH.exists(), "value_curves.csv not found; run compute_value_curves.py first."
...
def test_sample_positive():
    df = pd.read_csv(DATA_PATH)
    sample = df[(df["naics3"] == 722) & (df["geo_bucket"] == "metro_nyc") & (df["score_bucket_low"] == 40)]
