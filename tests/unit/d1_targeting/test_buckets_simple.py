...
class TestBucketsSimple:
    ...
    def test_geo_features_csv_exists_and_valid(self):
        ...
        required_cols = ['zip_code', 'city', 'state', 'affluence', 'agency_density', 'broadband_quality']
    ...
    def test_vertical_features_csv_exists_and_valid(self):
        ...
        # Check required columns
        required_cols = ['yelp_category', 'business_vertical', 'urgency', 'ticket_size', 'maturity']
        for col in required_cols:
            assert col in df.columns, f"Missing column: {col}"
    ...
    def test_vert_bucket_combinations_count(self):
        ...
        assert theoretical_combos >= 8, f"Should have at least 8 possible vertical combinations, got {theoretical_combos}"
