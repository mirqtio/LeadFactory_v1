...
class TestLineageAPIv2:
    ...
    def test_view_pipeline_logs(self, test_client: TestClient, sample_lineage_data, db_session):
        ...
        # Verify audit log
        audits = (
            db_session.query(ReportLineageAudit)
            .filter(ReportLineageAudit.lineage_id == lineage.id, ReportLineageAudit.action == "view_logs")
            .all()
        )
        assert len(audits) == 1
    ...
    def test_download_raw_inputs(self, test_client: TestClient, sample_lineage_data, db_session):
        ...
        audit = (
            db_session.query(ReportLineageAudit)
            .filter(ReportLineageAudit.lineage_id == lineage.id, ReportLineageAudit.action == "download_raw_inputs")
            .first()
        )
        assert audit is not None
    ...
    def test_delete_lineage(self, test_client: TestClient, sample_lineage_data, db_session):
        ...
        deleted = db_session.query(ReportLineage).filter(ReportLineage.id == lineage.id).first()
