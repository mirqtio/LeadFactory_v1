...
class TestEnvironmentConfig:
    ...
    def test_test_environment_forces_stubs(self):
        ...
        """
        with patch.dict(os.environ, {"ENVIRONMENT": "test", "USE_STUBS": "false"}):  # Try to disable stubs
            settings = Settings()
    ...
    def test_ci_environment_forces_stubs(self):
        ...
        """
        with patch.dict(os.environ, {"CI": "true", "USE_STUBS": "false"}):  # Try to disable stubs
            settings = Settings()
            assert settings.use_stubs is True  # Should be forced to True
    ...
    def test_development_allows_stub_choice(self):
        ...
        with patch.dict(
            os.environ,
            {"ENVIRONMENT": "development", "USE_STUBS": "true", "CI": ""},  # Make sure CI is not set
            clear=True,
        ):
            settings = Settings()
    ...
    def test_api_urls_redirect_to_stub_server(self):
        ...
        """
        with patch.dict(os.environ, {"USE_STUBS": "true", "STUB_BASE_URL": "http://localhost:5010"}):
            settings = Settings()
    ...
    def test_environment_variables_documented(self):
        ...
        important_fields = ["environment", "use_stubs", "stub_base_url", "database_url", "secret_key"]
