...
class TestTokenValidation:
    ...
    def test_color_value_format(self):
        ...
        for category_name, category in colors.items():
            for color_name, color_data in category.items():
                ...
                assert all(c in '0123456789abcdefABCDEF' for c in hex_chars), f"Invalid hex characters: {category_name}.{color_name}"
    ...
    def test_minified_json_format(self):
        ...
        if tokens_file.exists():
            ...
            assert ': {' not in content or content.count(': {') < 20, "Too many spaces after colons suggest non-minified format"
