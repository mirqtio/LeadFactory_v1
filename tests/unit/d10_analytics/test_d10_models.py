...
from d10_analytics.models import (
    FunnelEvent, MetricSnapshot, TimeSeriesData, DashboardMetric, FunnelConversion,
...
class TestAnalyticsModels:
    ...
    def test_funnel_event_creation(self, db_session, sample_funnel_event):
        ...
        saved_event = db_session.query(FunnelEvent).filter_by(
            event_id=sample_funnel_event.event_id
        ).first()
    ...
    def test_funnel_event_enums(self, db_session):
        ...
        stage_count = db_session.query(FunnelEvent).filter(
            FunnelEvent.event_name.like("test_%")
        ).count()
    ...
    def test_metric_snapshot_creation(self, db_session, sample_metric_snapshot):
        ...
        saved_snapshot = db_session.query(MetricSnapshot).filter_by(
            snapshot_id=sample_metric_snapshot.snapshot_id
        ).first()
    ...
    def test_metric_snapshot_aggregation_types(self, db_session):
        ...
        for metric_type in MetricType:
            ...
            period_end = datetime(period_start.year, period_start.month, period_start.day, 23, 59, 59, tzinfo=timezone.utc)
        ...
        metric_count = db_session.query(MetricSnapshot).filter(
            MetricSnapshot.metric_name.like("test_%")
        ).count()
    ...
    def test_time_series_data_creation(self, db_session):
        ...
        ts_points = db_session.query(TimeSeriesData).filter_by(
            metric_name="response_time_ms"
    ...
    def test_dashboard_metric_creation(self, db_session):
        ...
        saved_metric = db_session.query(DashboardMetric).filter_by(
            dashboard_name="operations_dashboard"
        ).first()
    ...
    def test_funnel_conversion_creation(self, db_session):
        ...
        saved_conversion = db_session.query(FunnelConversion).filter_by(
            from_stage=FunnelStage.ASSESSMENT,
            to_stage=FunnelStage.SCORING
        ).first()
    ...
    def test_efficient_indexing(self, db_session):
        ...
        stage_type_results = db_session.query(FunnelEvent).filter(
        ...
        business_time_results = db_session.query(FunnelEvent).filter(
            FunnelEvent.business_id == "business_1"
        ).order_by(FunnelEvent.occurred_at).all()
        assert len(business_time_results) > 0
        
        # Query by success status
        success_results = db_session.query(FunnelEvent).filter(
