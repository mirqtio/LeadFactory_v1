...
class TestSheetToYamlConverter:
    ...
    def mock_sheet_data(self):
        ...
        return {
            ...
            "components": {
                "company_info": {
                    "weight": 0.3,
                    "factors": {"name_quality": {"weight": 0.5}, "industry_classification": {"weight": 0.5}},
                },
                "online_presence": {
                    "weight": 0.4,
                    "factors": {"website_quality": {"weight": 0.6}, "ssl_certificate": {"weight": 0.4}},
                },
                "business_validation": {"weight": 0.3, "factors": {"license_verified": {"weight": 1.0}}},
            },
            "sha": "abc123",
        }
    ...
    def test_parse_tiers(self):
        ...
        sheet_values = [["tierA", "80", "A"], ["tierB", "60", "B"], ["tierC", "40", "C"], ["tierD", "0", "D"]]
    ...
    def test_validate_structure_bad_factor_weights(self):
        ...
        data = {
            ...
            "components": {
                "comp1": {
                    "weight": 1.0,
                    "factors": {"factor1": {"weight": 0.7}, "factor2": {"weight": 0.7}},  # Total = 1.4
                }
            },
        }
    ...
    def test_main_function(self, tmp_path, monkeypatch, mock_credentials):
        ...
        with patch("scripts.sheet_to_yaml.SheetToYamlConverter") as MockConverter:
            ...
            sys.argv = ["sheet_to_yaml.py", "--sheet-id", "test-id", "--tab", "test-tab", "--output", str(output_file)]
