...
class LeadFactoryUser(HttpUser):
    ...
    def get_business_assessment(self):
        ...
        with self.client.get(
            f"/api/v1/businesses/{business_id}/assessment",
            catch_response=True,
            name="get_assessment",
        ) as response:
            if response.status_code == 200:
    ...
    def get_business_score(self):
        ...
        with self.client.get(
            f"/api/v1/businesses/{business_id}/score",
            catch_response=True,
            name="get_score",
        ) as response:
            if response.status_code == 200:
    ...
    def create_targeting_criteria(self):
        ...
        with self.client.post(
            "/api/v1/targeting/criteria",
            json=target_data,
            catch_response=True,
            name="create_targeting",
        ) as response:
    ...
    def download_report(self):
        ...
        with self.client.get(
            f"/api/v1/reports/{business_id}/download",
            catch_response=True,
...
class AdminUser(HttpUser):
    ...
    def view_pipeline_status(self):
        ...
        with self.client.get(
            "/api/v1/admin/pipelines/status",
            catch_response=True,
            name="admin_pipeline_status",
        ) as response:
            if response.status_code == 200:
                try:
                    data = response.json()
...
def configure_load_test():
    ...
    # Standard load test
    standard_config = {"users": 50, "spawn_rate": 5, "duration": "5m"}

    # Stress test
    stress_config = {"users": 200, "spawn_rate": 20, "duration": "10m"}

    # Spike test
    spike_config = {"users": 500, "spawn_rate": 50, "duration": "2m"}

    return {"standard": standard_config, "stress": stress_config, "spike": spike_config}
