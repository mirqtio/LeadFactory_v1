...
def mock_external_services():
    ...
    with patch('d0_gateway.providers.yelp.YelpClient') as mock_yelp, \
         patch('d0_gateway.providers.pagespeed.PageSpeedClient') as mock_pagespeed, \
         patch('d0_gateway.providers.openai.OpenAIClient') as mock_openai, \
         patch('d0_gateway.providers.sendgrid.SendGridClient') as mock_sendgrid:
        
        # Configure mock responses for consistent performance testing
        mock_yelp.return_value.search_businesses.return_value = {
            'businesses': [{'id': f'test_yelp_{i}', 'name': f'Test Business {i}'} for i in range(100)]
        }
