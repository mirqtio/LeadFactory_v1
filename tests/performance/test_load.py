...
from database.models import (
    Business, Target, ScoringResult, Email, Purchase, 
    GatewayUsage, PurchaseStatus, EmailStatus, GeoType, Batch, BatchStatus
)
...
def test_response_time_measurement(test_db_session, mock_external_services):
    ...
    stage_results = {
        ...
        'assessment': {
            ...
            'p95': statistics.quantiles(assessment_times, n=20)[18] if len(assessment_times) >= 20 else max(assessment_times),
        ...
        'personalization': {
            ...
            'p95': statistics.quantiles(personalization_times, n=20)[18] if len(personalization_times) >= 20 else max(personalization_times),
...
def test_bottleneck_identification(test_db_session, mock_external_services):
    ...
    if bottleneck_percentage > 30:  # Lower threshold for testing
        recommendations.append(f"Optimize {bottleneck_stage} stage - consumes {bottleneck_percentage:.1f}% of processing time")
    ...
    for stage, analysis in bottleneck_analysis.items():
        if analysis['consistency'] < 0.8:  # High variability threshold
            recommendations.append(f"Investigate {stage} stage variability (consistency: {analysis['consistency']:.2f})")
    
    # Always provide at least one recommendation for bottleneck analysis
    if len(recommendations) == 0:
        recommendations.append(f"Primary bottleneck is {bottleneck_stage} stage ({bottleneck_percentage:.1f}% of total time)")
    ...
    print(f"  Memory: {resource_analysis['memory_utilization']:.1f}MB avg, {performance_metrics['peak_memory']:.1f}MB peak")
...
def test_resource_usage_tracking(test_db_session, mock_external_services):
    ...
    efficiency_metrics = {
        ...
        'memory_efficiency': len(businesses) / performance_metrics['avg_memory'] if performance_metrics['avg_memory'] > 0 else 0,
    ...
    assert resource_metrics['throughput'] > 10, f"Throughput too low: {resource_metrics['throughput']:.2f} businesses/second"
    assert db_metrics['inserts_per_second'] > 50, f"Database insert rate too low: {db_metrics['inserts_per_second']:.1f}/second"
