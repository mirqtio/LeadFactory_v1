...
from database.models import (
    Business, Email, EmailSuppression, Purchase, EmailClick,
    PurchaseStatus, EmailStatus
)
...
def test_data_privacy_compliance(test_db_session):
    ...
    suppression = EmailSuppression(
        ...
        reason="privacy_compliance_test",
        source="test_system"
    )
    ...
    all_controls = [access_controls, privacy_rights, data_transfer_compliance, breach_protocols]
...
def test_email_compliance(test_db_session):
    ...
    can_spam_requirements = {
        ...
        'truthful_subject': {
            ...
            'compliant': 'spam' not in compliant_email.subject.lower() and 'free' not in compliant_email.subject.lower()
        },
        'opt_out_processing': {
    ...
    content_compliance = {
        'no_spam_words': {
            'spam_indicators': ['FREE', 'URGENT', 'LIMITED TIME', 'CLICK NOW', 'GUARANTEED'],
            'found_spam_words': [],
            'compliant': True
        },
        'personalization': {
