...
async def test_formula_evaluation():
    ...
    # Test simple formula
    result = evaluator.evaluate_formula("=IF({score}>80,10,5)", {"score": 85})
    assert result == 10

    # Test complex formula
    result = evaluator.evaluate_formula("=MIN(10, {score} * {weight})", {"score": 15, "weight": 0.8})
    assert result == 10
...
async def test_hot_reload_mechanism():
    ...
    with tempfile.NamedTemporaryFile(mode="w", suffix=".yaml", delete=False) as f:
        config = {
            ...
            "components": {"test": {"weight": 1.0, "max_score": 10}},
        }
        yaml.dump(config, f)
        temp_path = f.name
