...
from d8_personalization.personalizer import (
    EmailPersonalizer, PersonalizationRequest, IssueExtractor
)
from d8_personalization.subject_lines import (
    SubjectLineGenerator, SubjectLineRequest
)
from d8_personalization.spam_checker import SpamScoreChecker
from d8_personalization.models import (
    EmailContentType, PersonalizationStrategy, ContentStrategy
)
...
class TestPersonalizationIntegration:
    ...
    def sample_assessment_data(self):
        ...
        return [
            ...
            },
            {
                ...
                'issues': {
                    'count': 5,
                    'list': ['slow_loading', 'broken_links', 'missing_ssl', 'seo_issues', 'mobile_unfriendly']
                },
                'techstack': {
                    'security_score': 0.3,
    ...
    def test_full_generation_flow(self, mock_openai_client, sample_business_data, sample_assessment_data, sample_contact_data):
    ...
    def test_quality_checks_pass(self, mock_openai_client, sample_business_data, sample_assessment_data, sample_contact_data):
        ...
        for result in results:
            ...
            assert result['spam_score'] <= 100.0, f"Spam score out of range for {result['business']}: {result['spam_score']}"
            assert result['content_length'] >= 50, f"Content too short for {result['business']}: {result['content_length']}"
    ...
    def test_performance_acceptable(self, mock_openai_client, sample_business_data, sample_assessment_data, sample_contact_data):
        ...
        for result in results:
            ...
            assert result['generation_time'] < 5.0, f"Generation too slow for {result['business']}: {result['generation_time']:.2f}s"
    ...
    def test_variety_in_output(self, mock_openai_client, sample_business_data, sample_assessment_data, sample_contact_data):
        ...
        async def test_output_variety():
            ...
            strategies = [
                (EmailContentType.COLD_OUTREACH, PersonalizationStrategy.WEBSITE_ISSUES, ContentStrategy.PROBLEM_AGITATION),
                (EmailContentType.AUDIT_OFFER, PersonalizationStrategy.BUSINESS_SPECIFIC, ContentStrategy.EDUCATIONAL_VALUE),
                (EmailContentType.FOLLOW_UP, PersonalizationStrategy.INDUSTRY_VERTICAL, ContentStrategy.BEFORE_AFTER),
            ]
            
            for i, (content_type, personalization_strategy, content_strategy) in enumerate(strategies):
                request = PersonalizationRequest(
        ...
        if len(strategy_outputs) >= 2:
            ...
            assert unique_strategy_approaches >= 2, f"Strategies not producing different content: {unique_strategy_approaches}"
        
        print(f"✓ Output variety verified - {unique_subjects}/{len(subjects)} unique subjects, {unique_html_hashes}/{len(html_contents)} unique HTML")
