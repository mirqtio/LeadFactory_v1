...
from d10_analytics.api import router, MetricsWarehouse
from d10_analytics.schemas import (
    MetricsRequest, FunnelMetricsRequest, CohortAnalysisRequest, ExportRequest,
...
class TestAnalyticsIntegration:
    ...
    def test_api_responses_correct(self, client, sample_funnel_data):
        ...
        with patch.object(MetricsWarehouse, 'calculate_funnel_conversions', new_callable=AsyncMock) as mock_funnel:
            ...
            required_fields = [
                'request_id', 'date_range', 'total_conversions', 
                'overall_conversion_rate', 'data', 'stage_summary', 
                'conversion_paths', 'generated_at'
            ]
    ...
    def test_data_consistency(self, client, sample_metrics_data, sample_funnel_data):
        ...
        with patch.object(MetricsWarehouse, 'get_daily_metrics', new_callable=AsyncMock) as mock_metrics, \
            ...
            assert cohort_data['total_cohorts'] == len(set((dp['cohort_date'], dp['campaign_id']) for dp in cohort_data['data']))
            ...
            for data_point in metrics_data['data']:
                if data_point['segment_breakdown']:
                    ...
                    assert 'campaign' in data_point['segment_breakdown'] or \
                           'vertical' in data_point['segment_breakdown'] or \
                           'region' in data_point['segment_breakdown']
    ...
    def test_health_check_integration(self, client):
        ...
        required_fields = ['status', 'version', 'uptime_seconds', 'dependencies', 'metrics_count']
        for field in required_fields:
