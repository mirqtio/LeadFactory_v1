...
class TestDeliveryIntegration:
    ...
    def test_full_send_flow_works(self, webhook_handler):
        ...
        with SessionLocal() as session:
            ...
            event = session.query(DeliveryEvent).filter(
    ...
    def test_compliance_verified(self, compliance_manager):
        ...
        with SessionLocal() as session:
            suppression = session.query(SuppressionList).filter(
                SuppressionList.email == suppressed_email
            ).first()
    ...
    def test_webhook_processing(self, webhook_handler):
        ...
        with SessionLocal() as session:
            ...
            events = session.query(DeliveryEvent).filter(
                DeliveryEvent.sendgrid_message_id == message_id
            ).order_by(DeliveryEvent.event_timestamp).all()
    ...
    def test_suppression_respected(self, webhook_handler):
        ...
        with SessionLocal() as session:
            ...
            bounce_tracking = session.query(BounceTracking).filter(
                BounceTracking.email == bounce_email
            ).first()
            ...
            bounce_suppression = session.query(SuppressionList).filter(
                SuppressionList.email == bounce_email
            ).first()
            ...
            spam_suppression = session.query(SuppressionList).filter(
                SuppressionList.email == spam_email
            ).first()
            ...
            bounce_delivery = session.query(EmailDelivery).filter(
                EmailDelivery.sendgrid_message_id == "bounce_msg_integration"
            ).first()
            assert bounce_delivery.status == DeliveryStatus.BOUNCED.value
            
            spam_delivery = session.query(EmailDelivery).filter(
                EmailDelivery.sendgrid_message_id == "spam_msg_integration"
            ).first()
    ...
    def test_duplicate_event_handling(self, webhook_handler):
        ...
        with SessionLocal() as session:
            event_count = session.query(DeliveryEvent).filter(
                DeliveryEvent.sendgrid_message_id == message_id,
