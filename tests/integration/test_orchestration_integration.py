...
from d11_orchestration.models import (
    PipelineRun, Experiment, ExperimentVariant, VariantAssignment, PipelineTask,
    PipelineRunStatus, ExperimentStatus, PipelineType, VariantType, generate_uuid
)
...
def test_db():
    ...
    from d11_orchestration.models import PipelineRun, Experiment, ExperimentVariant, VariantAssignment, PipelineTask
...
class TestPipelineEndToEnd:
    ...
    def test_pipeline_lifecycle_integration(self, test_db):
        ...
        completed_tasks = test_db.query(PipelineTask).filter(
...
class TestTaskExecution:
    ...
    def test_task_execution_order(self, test_db):
        ...
        # Create pipeline run
        pipeline_run = PipelineRun(
            pipeline_name="ordered_execution_pipeline",
            triggered_by="order_test"
        )
        ...
        ordered_tasks = test_db.query(PipelineTask).filter(
            PipelineTask.pipeline_run_id == pipeline_run.run_id
        ).order_by(PipelineTask.execution_order).all()
    ...
    def test_experiment_variant_assignment(self, test_db):
        ...
        for i in range(10):
            ...
            assignment = VariantAssignment(
                ...
                pipeline_run_id=pipeline_run.run_id,
                assignment_context={"step": "personalization", "pipeline_run": pipeline_run.run_id}
            )
        ...
        total_assignments = test_db.query(VariantAssignment).filter(
        ...
        control_assignments = test_db.query(VariantAssignment).filter(
            VariantAssignment.experiment_id == experiment.experiment_id,
            VariantAssignment.variant_id == control_variant.variant_id
        ).count()
        
        treatment_assignments = test_db.query(VariantAssignment).filter(
            VariantAssignment.experiment_id == experiment.experiment_id,
        ...
        pipeline_assignments = test_db.query(VariantAssignment).filter(
...
class TestMetricsRecording:
    ...
    def test_pipeline_metrics_integration(self, mock_metrics_class, test_db):
        ...
        tasks_with_time = test_db.query(PipelineTask).filter(
            PipelineTask.pipeline_run_id == pipeline_run.run_id,
    ...
    def test_experiment_metrics_correlation(self, test_db):
        ...
        experiment_pipelines = test_db.query(VariantAssignment).filter(
            VariantAssignment.experiment_id == experiment.experiment_id,
...
class TestOrchestrationIntegration:
    ...
    def test_complete_orchestration_workflow(self, test_db):
        ...
        control_count = test_db.query(VariantAssignment).filter(
            VariantAssignment.experiment_id == experiment.experiment_id,
            VariantAssignment.variant_id == control_variant.variant_id
        ).count()
        
        optimized_count = test_db.query(VariantAssignment).filter(
            VariantAssignment.experiment_id == experiment.experiment_id,
        ...
        experiment_pipeline_link = test_db.query(VariantAssignment).filter(
            VariantAssignment.pipeline_run_id == pipeline_run.run_id
        ).first()
