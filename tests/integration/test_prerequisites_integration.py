...
class TestPrerequisitesIntegration:
    ...
    def test_python_version_check_real(self):
        ...
        assert check.version_required == "3.11.0"
    ...
    def test_cli_interface_real(self):
        ...
        result = subprocess.run([
            sys.executable, "-m", "core.prerequisites"
        ...
        result_json = subprocess.run([
            sys.executable, "-m", "core.prerequisites", "--json"
        ], capture_output=True, text=True)
        ...
        result_specific = subprocess.run([
            sys.executable, "-m", "core.prerequisites", "--check", "python"
        ], capture_output=True, text=True)
        ...
        result_quiet = subprocess.run([
            sys.executable, "-m", "core.prerequisites", "--quiet"
        ], capture_output=True, text=True)
