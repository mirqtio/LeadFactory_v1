...
def test_post_launch_verification():
    ...
    ), f"System check JSON output failed: {json_result.stderr}"
    ...
    assert overall_status in [
        "excellent",
        "good",
    ], f"System status not acceptable: {overall_status}"
    ...
    for section in troubleshooting_sections:
        assert (
            section in troubleshooting_content
        ), f"Troubleshooting guide missing section: {section}"
    ...
    assert (
        len(failed_checks) == 0
    ), f"Production readiness checks failed: {failed_checks}"
