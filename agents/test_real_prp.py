...
def test_real_prp():
    ...
    pm_evidence = {
        ...
        if (k.decode() if isinstance(k, bytes) else k) in [
            "tests_passed", "coverage_pct", "lint_passed", 
            "implementation_complete", "files_modified"
        ]
    }
    ...
    val_evidence = {
        ...
        if (k.decode() if isinstance(k, bytes) else k) in [
            ...
            "performance_review", "validation_issues", "required_changes"
        ]
    }
    ...
    for k in ["implementation_complete", "tests_passed", "lint_passed", "coverage_pct",
              "validation_passed", "quality_score", "ci_passed", "deployed"]:
        if k in final_state:
