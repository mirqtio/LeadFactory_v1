...
class AgentWorker:
    ...
    def run(self):
        ...
        while True:
            try:
                ...
                prp_id = self.redis_client.blmove(
                    self.queue, 
                    f"{self.queue}:inflight",
                    timeout=5.0
                )
    ...
    def handle_completion(self, prp_id: str, evidence: Dict[str, str]):
        ...
        self.redis_client.hset(f"prp:{prp_id}", mapping={
            f"{self.role}_completed_at": completion_time,
            f"{self.role}_completed_by": self.agent_id
        })
    ...
    def update_agent_status(self, status: str, current_prp: str = ""):
        ...
        self.redis_client.hset(f"agent:{self.agent_id}", mapping={
            "status": status,
    ...
    def build_continuation_context(self, prp_id: str, previous_response: str) -> Dict[str, Any]:
        ...
        return {
            "messages": self.conversation_history + [
                {"role": "assistant", "content": previous_response},
                {"role": "user", "content": "Please continue with the task. If complete, output the evidence JSON blocks."}
            ]
        }