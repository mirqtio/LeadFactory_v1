...
class QAOrchestrator:
            ...
            try:
                ...
                if request:
                    ...
                    request_json = json.loads(
                        request_data.decode() if isinstance(request_data, bytes) else request_data
                    )
    ...
    def process_qa_request(self, request: Dict):
        ...
        try:
            ...
            self.redis_client.setex(f"qa_answer:{qa_id}", 300, answer)  # 5 minute TTL
    ...
    def get_codebase_context(self) -> Dict[str, str]:
        ...
        if self.codebase_cache and self.cache_timestamp and time.time() - self.cache_timestamp < self.cache_ttl:
            return self.codebase_cache

        self.logger.info("Building codebase context cache")

        context = {"structure": self.get_project_structure(), "key_files": {}, "documentation": {}, "configuration": {}}
        ...
        config_files = ["Makefile", "requirements.txt", "docker-compose.yml", ".github/workflows/ci.yml"]
        ...
        key_patterns = ["agents/core/*.py", "agents/roles/*.py", "bin/orchestrator*.py", "bin/enterprise_shim*.py"]
    ...
    def get_prp_context(self, prp_id: str) -> Dict:
        ...
        return {"data": prp_dict, "history": history}
    ...
    def get_role_history(self, prp_id: str, role: str) -> List[Dict]:
        ...
        for entry in raw_history:
            try:
                ...
                history.append({"timestamp": data.get("timestamp"), "summary": data.get("response", "")[:500] + "..."})
            except:
                pass
    ...
    def _summarize_docs(self, docs: Dict[str, str]) -> str:
        ...
        for file_path, content in docs.items():
            ...
            key_lines = [
                line
                for line in lines
                if line.strip() and (line.startswith("#") or line.startswith("**") or "IMPORTANT" in line)
            ]
            summary.append(f"\n{file_path}:\n" + "\n".join(key_lines[:20]))
