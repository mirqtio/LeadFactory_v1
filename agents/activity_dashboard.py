...
def render_activity_dashboard():
    ...
    if total_items == 0:
        ...
        print(
            f"  📋 {queues['new_queue']} new → 💻 {queues['dev_queue']} dev → 🔍 {queues['validation_queue']} validation → 🚀 {queues['integration_queue']} integration"
        )
    print()
    ...
    for agent_id, health in agent_health.items():
        ...
        health_icon = {"active": "🟢", "idle": "🟡", "stale": "🔴", "unknown": "❓", "error": "💥"}.get(health, "❓")
