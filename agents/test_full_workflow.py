...
def test_full_workflow():
    ...
    logger.info(f"After Validator: integration_queue = {[v.decode() if isinstance(v, bytes) else v for v in integration_queue]}")
    ...
    if is_complete and has_pm_completion and has_validator_completion and has_integration_completion:
        ...
        for k, v in sorted(final_state.items()):
            if k not in ["content", "title", "id"] and ("evidence" in k.lower() or "complete" in k.lower() or "passed" in k.lower()):
