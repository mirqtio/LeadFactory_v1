...
def test_pm_workflow():
    ...
    def run_agent():
        try:
            ...
            prp_id = agent.redis_client.blmove(agent.queue, f"{agent.queue}:inflight", timeout=5.0)
    ...
    while time.time() - start_time < timeout:
        ...
        if prp_id.encode() in validator_queue or prp_id in [
            v.decode() if isinstance(v, bytes) else v for v in validator_queue
        ]:
        ...
        pm_evidence = {
            ...
            if k in ["tests_passed", "coverage_pct", "lint_passed", "implementation_complete"]
        }
        if pm_evidence:
