...
class TestIntegrationAgent:
        ...
        with patch('redis.from_url', return_value=mock_redis):
            with patch('anthropic.Anthropic', return_value=mock_claude):
                ...
                mock_redis.hset(f"prp:{prp_id}", {
                    ...
                    "modified_files": json.dumps(["feature.py"])
                })
    ...
    def test_deployment_failure(self):
        ...
        evidence = {
            "ci_passed": "false",
            "ci_failure_reason": "Tests failed in CI"
        }
    ...
    def test_context_building(self):
        ...
        with patch('redis.from_url', return_value=mock_redis):
            with patch('anthropic.Anthropic', return_value=MockClaudeClient()):
                ...
                # Build context
                context = orchestrator.build_qa_context(
                    "Test question",
                    "pm",
                    "P0-009"
                )
