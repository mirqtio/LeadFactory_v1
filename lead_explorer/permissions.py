...
class BadgePermissions:
    ...
    SYSTEM_BADGE_PERMISSIONS = {
        ...
        "sales_rep": {"badge": ["read"], "lead_badge": ["create", "read", "update"], "system_badge": ["read"]},
        "analyst": {"badge": ["read"], "lead_badge": ["read"], "system_badge": ["read"]},
        "admin": {
            "badge": ["create", "read", "update", "delete"],
            "lead_badge": ["create", "read", "update", "delete"],
...
def check_badge_permission(user: AccountUser, action: str, resource: str, db: Session) -> bool:
    ...
    logger.info(
        f"Permission check: user={user.email}, role={user_role}, action={action}, resource={resource}, granted={has_permission}"
    )
...
def check_badge_type_permission(user: AccountUser, badge_type: str, db: Session) -> bool:
    ...
    logger.info(
        f"Badge type permission check: user={user.email}, role={user_role}, badge_type={badge_type}, granted={has_permission}"
    )
    ...
    def decorator(func):
        def wrapper(*args, **kwargs):
            ...
            if not user or not db:
                raise HTTPException(
                    status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Missing user or database context"
                )
...
class BadgePermissionMiddleware:
    ...
    def check_create_badge_permission(user: AccountUser, badge_data: dict, db: Session) -> None:
        ...
        if not check_badge_permission(user, "create", "badge", db):
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN, detail="Insufficient permissions to create badges"
            )
        ...
        if badge_type and not check_badge_type_permission(user, badge_type, db):
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN, detail=f"Cannot create badges of type '{badge_type}'"
            )
        ...
        if is_system and not check_badge_permission(user, "create", "system_badge", db):
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN, detail="Insufficient permissions to create system badges"
            )
    ...
    def check_assign_badge_permission(user: AccountUser, badge_id: str, lead_id: str, db: Session) -> None:
        ...
        if not check_badge_permission(user, "create", "lead_badge", db):
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN, detail="Insufficient permissions to assign badges"
            )
        ...
        if not check_badge_type_permission(user, badge.badge_type.value, db):
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN, detail=f"Cannot assign badges of type '{badge.badge_type.value}'"
            )
