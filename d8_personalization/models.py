...
class PersonalizationVariable(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    token_id = Column(UUID(as_uuid=True), ForeignKey('personalization_tokens.id'), nullable=False)
...
class SpamScoreTracking(Base):
    ...
    id = Column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    email_content_id = Column(UUID(as_uuid=True), ForeignKey('email_content.id'), nullable=False)
