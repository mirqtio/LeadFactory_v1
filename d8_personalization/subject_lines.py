...
from .models import (
    SubjectLineVariant, EmailTemplate, PersonalizationToken,
    PersonalizationVariable, EmailContentType, PersonalizationStrategy,
    VariantStatus, ContentStrategy
)
...
class SubjectLineGenerator:
    ...
    def _generate_from_template(
        ...
        # Resolve tokens
        tokens_resolved, tokens_failed = self._resolve_tokens(
            required_tokens, request
        )
        ...
        min_length = self.templates.get("generation_rules", {}).get(
            "global_constraints", {}
        ).get("min_length", 10)
    ...
    def _resolve_single_token(
        ...
        if token == "business_name" and request.business_data:
            ...
            value = (request.contact_data.get("first_name") or 
                    request.contact_data.get("name") or
                    request.contact_data.get("contact_name"))
        elif token == "industry" and request.business_data:
            value = (request.business_data.get("category") or
                    request.business_data.get("industry") or
                    request.business_data.get("vertical"))
        elif token == "location" and request.business_data:
            location_data = request.business_data.get("location", {})
            if isinstance(location_data, dict):
                value = (location_data.get("city") or
                        location_data.get("location") or
                        request.business_data.get("city"))
            else:
    ...
    def _calculate_spam_risk_score(self, subject_text: str) -> float:
        ...
        spam_words = self.templates.get("generation_rules", {}).get(
            "global_constraints", {}
        ...
        number_symbol_ratio = sum(
            1 for char in subject_text 
            if char.isdigit() or char in "$%@#"
        ) / len(subject_text) if subject_text else 0
    ...
    def _count_personalization_tokens(self, text: str) -> int:
        ...
        likely_personalized = sum(
            ...
            if word[0].isupper() and len(word) > 3 and word not in [
                "Quick", "About", "Website", "Your", "Free", "New"
            ]
        )
...
class SubjectLineManager:
    ...
    def get_best_performing_variant(
        ...
        variants = session.query(SubjectLineVariant).filter(
