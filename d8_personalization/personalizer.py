...
try:
    ...
    OpenAIClient = None
from .models import (
    EmailTemplate, EmailContent, PersonalizationStrategy, 
    ContentStrategy, SpamScoreTracking, EmailGenerationLog,
    EmailContentType, determine_risk_level
)
...
class IssueExtractor:
    ...
    def extract_issues_from_assessment(
        ...
        issues.sort(key=lambda x: (
            self._impact_to_score(x.impact),
            x.score
        ), reverse=True)
...
class SpamChecker:
    ...
    def __init__(self):
        self.spam_words = [
            # Common spam words
            'free', 'urgent', 'limited time', 'act now', 'exclusive', 'guarantee',
            'no obligation', 'risk free', 'winner', 'congratulations', 'click here',
            'buy now', 'order now', 'instant', 'immediately', 'cash', 'money',
            'earn money', 'make money', 'income', 'profit', 'investment',
            # Marketing spam
            'special promotion', 'limited offer', 'expires', 'deadline', 'hurry',
            'don\'t miss', 'once in a lifetime', 'incredible deal', 'amazing',
            'revolutionary', 'breakthrough', 'miracle', 'secret', 'hidden',
            # Urgency spam
            'urgent response', 'immediate action', 'time sensitive', 'expires today',
            'last chance', 'final notice', 'don\'t delay', 'while supplies last'
        ]
...
If you'd prefer not to receive these insights, please reply with "unsubscribe".
    ...
    def _calculate_quality_metrics(
        ...
        cta_indicators = ['schedule', 'call', 'reply', 'contact', 'consultation', 'discuss']
        cta_count = sum(1 for indicator in cta_indicators if indicator in text_content.lower())
        cta_score = min(cta_count * 0.3, 1.0)
        
        # Overall quality score
        overall_score = (length_score * 0.3 + personalization_score * 0.3 + 
                        readability_score * 0.2 + cta_score * 0.2)
